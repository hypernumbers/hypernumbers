...
;; Amended by Gordon Guthrie to provide Vixo specific headers
;; November 2005
...
A useful function is `tempo-template-erlang-norerl -s compile_code start -noshell`
mal-header'.
...
erl -s compile_code start -noshell`
...
(defvar erlang-electric-newline-inhibit-list
  '(erlang-electric-semicolon
    erlang-electric-comma
    erlang-electric-gt)
  "*Commands which c Func_call#fun_call{mod   = 'fakemod', 
%%%                    func  = 'fakefunc', 
%%%                    arity =3}. 
%%% Param_list=[Param1, Param2, Param3], 
%%% Write_tab_rec = Params#params{func_call_rec = Fun_call, 
%%%                               time_of_call = calendar:now_to_datetime(now()), 
%%%                               param_list = Param_list}. 
%%% ets:insert(write_table, Write_tab_rec),an inhibit the next newline.")
...
(defvar erlang-skel
  '(("If"            "if"            erlang-skel-if)
    ...
    ("Fix Me"        "fixme"         erlang-skel-fixme)
    ("Bodge"         "bodge"         erlang-skel-bodge)
    ()
    ...
    ("Vixo Erlang test suite TS frontend" "vixo-ts-test-suite"
     erlang-skel-ts-test-suite-vixo erlang-skel-header)
...
"%%%                                                           %%%" n 
"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" n)
"*The skeleton of a `fixme' attribute.
Please see the function `tempo-define-template'.")

(defvar erlang-skel-bodge
  '(&
"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" n
"%%%                 ____            _                         %%%" n
"%%%                |  _ \          | |                        %%%" n
...
"%%%                |  _ < / _ \ / _` |/ _` |/ _ \             %%%" n
"%%%                | |_) | (_) | (_| | (_| |  __/             %%%" n
"%%%                |____/ \___/ \__,_|\__, |\___|             %%%" n
"%%%                                    __/ |                  %%%" n
"%%%                                   |___/                   %%%" n
"%%%                                                           %%%" n
"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" n)
"*The skeleton of a `bodge' attribute.
Please see the function `tempo-define-template'.")
...
(defvar erlang-skel-large-ts-header-vixo
  '(o (erlang-skel-separator)
      (erlang-skel-include erlang-skel-copyright-comment
			   erlang-skel-file-comment
			   erlang-skel-author-comment)
      ...
      (erlang-skel-separator)
      "%%% " n
      "%%% This module is used to handle input and output for  " n
      "%%% the generic test frameworks. " n
      "%%% " n
      "%%% It creats the following ets tables in init_per_suite: " n
      "%%% * write_table " n
      "%%% * read_table " n
      "%%% " n
      "%%% It also deletes those ets tables in end_per_suite " n
      "%%% " n
      "%%% " n
      "%%% Where the contents of a function call need to be recorded by the " n
      "%%% generic test framework the contents should be written with the record " n
      "%%% of type 'parameter' defined in test_framework.hrl " n
      "%%% " n
      "%%% The first parameter of this record should be a record of type " n
      "%%% 'function_call' also defined in test_framework.hrl " n
      "%%% " n
      "%%% Each time the function is called it should write its parameter values " n
      "%%% to the ets table. The parameters should be compressed a list of values " n
      "%%% ordered by the order they appear in the function call " n
      "%%% " n
      "%%% For example is the test_framework is using a fake module called 'fakemod' " n
      "%%% and the code we are calling calls the function 'fakefunc/3' in the " n
      "%%% following format: " n
      "%%% " n
      "%%%    (...) " n
      "%%%    fakemod:fakefunc(Param1, Param2, Param3) " n
      "%%%    (...) " n
      "%%% " n
      "%%% The head clauses should be numbered in the test framework in the " n
      "%%% The the test framework should then write to the table 'write_table' in the " n
      "%%% following way " n
      "%%% " n
      "%%%    Func_call#func_call{mod  = 'fakemod', " n
      "%%%                       func  = 'fakefunc', " n
      "%%%                       arity = 3 }, " n
      "%%%    Param_list=[Param1, Param2, Param3], " n
      "%%%    Write_tab_rec = Params#params{func_call_rec = Func_call, " n
      "%%%                                  time_of_call = calendar:now_to_datetime(now()), " n
      "%%%                               param_list = Param_list}, " n
      "%%%    ets:insert(write_table, Write_tab_rec), " n
      "%%% " n
      "%%% Note that the key field of the record type 'params' is used for the  " n
      "%%% write table " n
      "%%% " n
      "%%% The tests can then read out the function calls made with the following code " n
      "%%% " n
      "%%%    Key = #fun_call{mod   = 'fakemod', " n
      "%%%                    func  = 'fakefunc', " n
      "%%%                    arity = 3}, " n
      "%%%    Result = ets:lookup(write_table, Key), " n
      "%%% " n
      "%%% " n
      "%%% Likewise the read_table can be used to return values from the framework " n
      "%%% " n
      "%%% In this case the read_table needs to be populated by the the appropriate " n
      "%%% test_suite functions, eg: " n
      "%%% * init_per_suite/1 " n
      "%%% * init_per_test/1 " n
      "%%% " n
      "%%% In this case the value 'key' of the record type 'params' is used to store " n
      "%%% state - the test framework must pull records out of the table based upon " n
      "%%% the parameters passed in " n
      "%%% " n
      (erlang-skel-separator) 
      (erlang-skel-include erlang-skel-small-header) )
  "*The template of a large test suite header (vixo).
Please see the function `tempo-define-template'.")
...
(defvar erlang-skel-ts-test-suite-vixo
 '((erlang-skel-include erlang-skel-large-ts-header-vixo)
   "%% Note: This directive should only be used in test suites." n
    "-compile(export_all)." n n

    "-include(\"test_server.hrl\")." n n
    "-include(\"test_framework.hrl\"). " n n
    ...
    "%% " n
    "%% For use with a test framework this function creates a pair of ets tables " n
    "%% that can be used to store data passed into the test framework and to " n
    "%% provide data that is to be passed back from the test framework " n
    "%% " n
    "%% In theory you create and delete these ets tables in the init_per_test " n
    "%% function but the standard is to create a new test suite " n
    "%% " n
    "%%*********************************************************************** " n
    "%%*********************************************************************** " n
    "%% Addition paths are needed here to ensure that the appropriate dummy ** " n
    "%% application framework is loaded instead of the real one             ** " n
    "%%*********************************************************************** " n
    "%%*********************************************************************** " n
    "%% " n
    (erlang-skel-separator 2) n
    "%% Test server callback functions" n n

    "init_per_suite(Config) ->" n >
    "Tid1 = ets:new(write_table, [duplicate_bag, public, named_table]), " n >
    "Tid2 = ets:new(read_table, [duplicate_bag, public, named_table])," n
    "Config." n n
    ...
    "true = ets:delete(write_table)," n >
    "true = ets:delete(read_table)," n
    "ok." n n
