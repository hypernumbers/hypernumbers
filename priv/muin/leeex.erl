...
parse_rules(_Ifile, {ok,_Other,L}, _Ms) ->
    {error,{L,leex,missing_rules}};
parse_rules(_Ifile, {eof,L}, _Ms) ->
    {error,{L,leex,missing_rules}}.
...
collect_rule(Ifile, Chars, L0) ->
	...
	{ok,[{':',_Lc}|Toks],L1} -> {ok,string:substr(Chars, St, Len),Toks,L1};
	{ok,_Toks,_L1} -> {error,{L0,leex,bad_rule}};
...
collect_rule(Ifile, Chars, L0, Cont0) ->
	...
	{done,{ok,Toks,_L1},_Rest} -> {ok,Toks,L0};
	{done,{eof,_L1},_Rest} -> {eof,L0};
	{done,{error,E,_L1},_Rest} -> {error,E,L0};
	{more,Cont1} ->
...
parse_rules(_Ifile, {eof,_Line}, _Ms, REAs, As, _N) ->
    %% Must be careful to put rules in correct order!
...
parse_rule(S, Line, [{dot,_Ld}], Ms, N) ->
...
parse_rule_regexp(RE0, [{M,Exp}|Ms]) ->
	...
	{ok,RE,_N} -> parse_rule_regexp(RE, Ms);
	{error,_E} -> parse_rule_regexp(RE0, Ms)
    end;
...
parse_code(Ifile, _Line, REAs, As) ->
...
min_use(Old, [{Old,New}|_Reds]) -> New;
min_use(Old, [_R|Reds]) -> min_use(Old, Reds);
min_use(Old, []) -> Old.
...
out_body(File, Next, Line, _C, Action) ->
    io:fwrite(File, "    yystate(~w, Ics, ~s, Tlen+1, ~w, Tlen);\n",
	      [Next,Line,Action]).

out_body(File, Next, Line, _C) ->
