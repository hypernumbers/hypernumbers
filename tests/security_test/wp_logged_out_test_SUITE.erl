...
%%%-----------------------------------------------------------------------------
-module(wp_logged_out_test_SUITE).
...
?test(wp_test4, "/failpath/", "{\"set\":{\"mark\":\"mark message\"}}", 401).
...
%  when O == page ->
?test(wp_test11, "/failpath/", "{\"delete\":\"all\"}", 401).

%ipost(Ref, _Qry,
%      Env=#env{body=[{"delete", "all"}],
%               uid = Uid}) ->
?test(wp_test12a, "/failpath/A1", "{\"delete\":\"all\"}", 401).
?test(wp_test12b, "/failpath/A1:B1", "{\"delete\":\"all\"}", 401).
?test(wp_test12c, "/failpath/A:A", "{\"delete\":\"all\"}", 401).
?test(wp_test12d, "/failpath/1:1", "{\"delete\":\"all\"}", 401).
...
?test(wp_test13, "/failpath/A1", "{\"delete\":\"horizonal\"}", 401).
...
%  when O == cell orelse O == range,
%       Direction == "horizontal" orelse Direction == "vertical" ->
?test(wp_test14, "/failpath/A1", "{\"insert\":\"vertical\"}", 401).

%ipost(Ref,
%      _Qry,
%      Env=#env{body=[{"copy", {struct, [{"src", Src}]}}],
%               uid = Uid}) ->
?test(wp_test15, "/failpath/A1", "{\"copy\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 401).

%ipost(Ref,
%      _Qry,
%      Env=#env{body=[{"copystyle", {struct, [{"src", Src}]}}],
%               uid = Uid}) ->
?test(wp_test16, "/failpath/A1", "{\"copystyle\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 401).

%ipost(Ref,
%      _Qry,
%      Env=#env{body=[{"copyvalue", {struct, [{"src", Src}]}}],
%               uid = Uid}) ->
?test(wp_test17, "/failpath/A1", "{\"copyvalue\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 401).

%ipost(#refX{obj = {range, _}} = Ref, _Qry,
%      Env=#env{body=[{"borders", {struct, Attrs}}]}) ->
?test(wp_test18, "/failpath/", "{\"borders\":{\"where\":\"top\",\"border\":\"none\",\"border_style\":\"thin\",\"border_color\":\"#ff0000\"}}", 401).

%ipost(#refX{site = _Site, path=["_user"]}, _Qry,
%      _Env=#env{body = [{"set", {struct, [{"language", _Lang}]}}],
%               uid = _Uid}) ->
?test(wp_test19, "/failpath/", "{\"set\":{\"language\":\"de\"}}", 401).

%ipost(Ref=#refX{path = P} = Ref, _Qry,
%      Env=#env{body = [{"postform", {struct, Vals}}], uid = PosterUid}) ->
?test(wp_test20, "/failpath/", "{\"postform\":{\"results\":\"./replies/\",\"values\":[{\"label\":\"label1\",\"formula\":\"hey!\"}]}}", 401).

%ipost(Ref, _Qry, Env=#env{body = [{"set", {struct, Attr}}], uid = Uid}) ->
?test(wp_test21, "/failpath/", "{\"set\":{\"bleh\":\"bloh\"}}", 401).

%ipost(Ref, _Qry, Env=#env{body = [{"clear", What}], uid = Uid})
%  when What == "contents"; What == "style"; What == "all" ->
?test(wp_test22, "/failpath/", "{\"clear\":\"contents\"}", 401).

%ipost(Ref, _Qry, Env=#env{body = [{"clear", What}], uid = Uid}) ->
?test(wp_test23, "/failpath/", "{\"clear\": \"arbritrary\"}", 401).

%ipost(#refX{site = Site, path = _P}, _Qry,
%      Env=#env{body = [{"admin", Json}], uid = Uid}) ->
?test(wp_test24, "/failpath/", "{\"admin\": {\"set_champion\": {\"path\":\"/blah/\", \"view\":\"spreadsheet\"}}}", 401).
