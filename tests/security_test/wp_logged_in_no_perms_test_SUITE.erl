...
%               uid = Uid}) ->
?test(wp_test15, "/401path/A1", "{\"copy\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 401).

%ipost(Ref,
%      _Qry,
%      Env=#env{body=[{"copystyle", {struct, [{"src", Src}]}}],
%               uid = Uid}) ->
?test(wp_test16, "/401path/A1", "{\"copystyle\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 401).

%ipost(Ref,
%      _Qry,
%      Env=#env{body=[{"copyvalue", {struct, [{"src", Src}]}}],
%               uid = Uid}) ->
?test(wp_test17, "/401path/A1", "{\"copyvalue\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 401).
