...
%               uid = Uid})
?test(wp_test1a, "/test1/A1", "{\"drag\":{\"range\": \"A4:B9\"}}", 200).
?test(wp_test1b, "/test1/A1:A1", "{\"drag\":{\"range\": \"A4:B9\"}}", 401).
?test(wp_test1c, "/test1/A:A", "{\"drag\":{\"range\": \"A4:B9\"}}", 401).
?test(wp_test1d, "/test1/1:1", "{\"drag\":{\"range\": \"A4:B9\"}}", 401).
?test(wp_test1e, "/test1/", "{\"drag\":{\"range\": \"A4:B9\"}}", 401).
...
%ipost(_Ref, #qry{mark = []},
%      Env=#env{body = {"mark", _Msg}) ->
?test(wp_test4, "/test1/", "{\"mark\":\"mark message\"}", 200).
...
%               uid = Uid}) ->
?test(wp_test15a, "/test1/A1", "{\"copy\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 200).
?test(wp_test15b, "/test1/A1:B2", "{\"copy\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 200).

%ipost(Ref,
%      _Qry,
%      Env=#env{body=[{"copystyle", {struct, [{"src", Src}]}}],
%               uid = Uid}) ->
?test(wp_test16a, "/test1/A1", "{\"copystyle\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 200).
?test(wp_test16b, "/test1/A1:B2", "{\"copystyle\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/D11\"}}", 200).

%ipost(Ref,
%      _Qry,
%      Env=#env{body=[{"copyvalue", {struct, [{"src", Src}]}}],
%               uid = Uid}) ->
?test(wp_test17a, "/test1/A1", "{\"copyvalue\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/b1\"}}", 200).
?test(wp_test17b, "/test1/A1:B3", "{\"copyvalue\":{\"src\": \"http://security.hypernumbers.dev:9000/test1/D11\"}}", 200).
...
?test(wp_test21a, "/test1/", "{\"set\":{\"bleh\":\"bloh\"}}", 401).
?test(wp_test21b, "/test1/a1", "{\"set\":{\"bleh\":\"bloh\"}}", 200).
?test(wp_test21c, "/test1/a1:b5", "{\"set\":{\"bleh\":\"bloh\"}}", 200).
?test(wp_test21d, "/test1/a:a", "{\"set\":{\"bleh\":\"bloh\"}}", 401).
?test(wp_test21e, "/test1/1:2", "{\"set\":{\"bleh\":\"bloh\"}}", 401).
