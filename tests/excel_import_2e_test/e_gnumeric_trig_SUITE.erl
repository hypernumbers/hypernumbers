        ...
        case Res of
            ...
            false ->
                ...
                exit("FAIL: Mismatch in ~p in ~p~n", 
                     [Testcase, "e_gnumeric_trig_SUITE %>"])
        end).

-define(test(Func, Path, Ref, Expected),
        Func(_Config) ->
               E = Expected,
               G = hnget("/" ++ "e_gnumeric_trig" ++ Path, Ref),
               Res = cmp(G, E),
...
?test(sheet1_A4, "/Summary/", "A4", "Trigonometry").
?test(sheet1_B4, "/Summary/", "B4", "Success").
?test(sheet1_A5, "/Summary/", "A5", "ATan2").
?test(sheet1_B5, "/Summary/", "B5", "Success").
?test(sheet1_A6, "/Summary/", "A6", "PI").
?test(sheet1_B6, "/Summary/", "B6", 3.14159265358979).
?test(sheet1_C6, "/Summary/", "C6", "PI").
?test(sheet1_D6, "/Summary/", "D6", 3.14159265358979).
?test(sheet1_E6, "/Summary/", "E6", "Success").
%% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
init_per_suite(Config) ->
    ...
    Postcell =
        fun({{{sheet, Sheetname}, {row_index, Row}, {col_index, Col}}, Val}) ->
                ...
                Path = "/" ++ "e_gnumeric_trig" ++ "/" ++ Sheetname ++ "/",
