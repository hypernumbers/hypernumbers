        ...
        case Res of
            ...
            false ->
                ...
                exit("FAIL: Mismatch in ~p in ~p~n", 
                     [Testcase, "b_multiple_sheets_SUITE %>"])
        end).

-define(test(Func, Path, Ref, Expected),
        Func(_Config) ->
               E = Expected,
               G = hnget("/" ++ "b_multiple_sheets" ++ Path, Ref),
               Res = cmp(G, E),
               ?print_error_or_return(Res, Func)).

%% TESTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
?test(sheet1_A1, "/Tom/", "A1", "Tom").
?test(sheet1_A2, "/Tom/", "A2", "Thomas").
?test(sheet2_A1, "/Dick/", "A1", "Dick").
?test(sheet3_A1, "/Harry/", "A1", "Harry").
%% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
init_per_suite(Config) ->
    ...
    Celldata = test_util:readxls("../../excel_files/Win_Excel07_As_97/" ++
                                 "b_multiple_sheets.xls"),
    io:format("DATA:~n~p~n~n", [Celldata]),
    Postcell =
        fun({{{sheet, Sheetname}, {row_index, Row}, {col_index, Col}}, Val}) ->
                Postdata = conv_for_post(Val),
                Path = "/" ++ "b_multiple_sheets" ++ "/" ++ Sheetname ++ "/",
