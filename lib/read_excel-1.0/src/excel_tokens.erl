...
parse_tokens(?tExp,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{expr_formula_range,{tExp,[{sheet,Name},{row_index,RowIndex},
						{col_index,ColIndex}],
...
parse_tokens(?tTbl,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     ColIndex:16/little-unsigned-integer,
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{table_formula_range,{tRbl,[{row_index,RowIndex},
						 {col_index,ColIndex}],
					   {return,none}}}|Residuum]);
%% tAdd
parse_tokens(?tAdd,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{addition,{tAdd,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tSub
parse_tokens(?tSub,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{subtraction,{tSub,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tMul
parse_tokens(?tMul,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{multiply,{tMul,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tDiv
parse_tokens(?tDiv,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{divide,{tDiv,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tPower
parse_tokens(?tPower,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{power,{tPower,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tConcat
parse_tokens(?tConcat,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{concatenate,{tConcat,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tLT
parse_tokens(?tLT,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{less_than,{tLT,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tLE
parse_tokens(?tLE,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{less_than_or_equal,{tLE,[{op_type,binary}],
					  {return,value}}}|Residuum]);

%% tEQ
parse_tokens(?tEQ,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{equals,{tEQ,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tGE
parse_tokens(?tGE,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{greater_than_or_equal,{tGE,[{op_type,binary}],
					      {return,value}}}
		     |Residuum]);
...
parse_tokens(?tGT,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{greater_than,{tGT,[{op_type,binary}],{return,value}}}
		     |Residuum]);

%% tNE
parse_tokens(?tNE,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{not_equal,{tNE,{op_type,binary},{return,value}}}
		     |Residuum]);

%% tIsect
parse_tokens(?tIsect,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{intersect,{tIsect,[{op_type,binary}],{return,reference}}}
		     |Residuum]);

%% tList
parse_tokens(?tList,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{list,{tList,[{op_type,binary}],{return,reference}}}
		     |Residuum]);
%% tRange
parse_tokens(?tRange,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{range,{tRange,[{op_type,binary}],{return,reference}}}
		     |Residuum]);
%% tUplus
parse_tokens(?tUplus,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{plus,{tUplus,[{op_type,unary}],{return,value}}}|Residuum]);
    
%% tUminus
parse_tokens(?tUminus,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{minus,{tUminus,[{op_type,unary}],{return,value}}}
		     |Residuum]);
%% tPercent
parse_tokens(?tPercent,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{percent,{tPercent,[{op_type,unary}],{return,value}}}
		     |Residuum]);
%% tParen
parse_tokens(?tParen,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{parentheses,{tParen,[],{return,none}}}|Residuum]);

%% tMissArg
parse_tokens(?tMissArg,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{missing_argument,{tMissArg,[],{return,value}}}|Residuum]);
...
parse_tokens(?tStr,Name,Bin,TokenArray,Residuum)->
    ...
    parse_tokens(R4,Name,TokenArray,[{string,{tStr,[{value,String3}],{return,value}}}|Residuum]);
...
parse_tokens(?tErr,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{error,{tErr,[{value,Error}],{return,none}}}
		     |Residuum]);
...
parse_tokens(?tBool,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{boolean,{tBool,[{value,Bool}],{return,value}}}
		     |Residuum]);
...
parse_tokens(?tInt,Name,Bin,TokenArray,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{integer,{tInt,[{value,Int}],{return,value}}}
		     |Residuum]);
...
parse_tokens(?tNum,Name,Bin,TokenArray,Residuum)->
    ...
    parse_tokens(R2,Name,TokenArray,[{number,{tNum,[{value,Num}],{return,value}}}
		     |Residuum]);
...
parse_tokens(tArray,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{array_type,{tArray,[{type,Type}],{return,array}}}
		     |Residuum]);
...
parse_tokens(tFunc,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{functional_index,{tFunc,[{value,Func},{type,Type}],
...
parse_tokens(tName,Name,Bin,TokenArray,Type,Residuum)->
    ...
    parse_tokens(R2,Name,TokenArray,[{name_index,{tName,[{value,Index-1},{type,Type}],
...
parse_tokens(tRef,Name,Bin,TokenArray,Type,Residuum)->
    ...
    parse_tokens(R2,Name,TokenArray,[{abs_ref,{tRef,[{value,ParsedAddy}|{type,Type}],
					  {return,reference}}}
		     |Residuum]);
...
parse_tokens(tArea,Name,Bin,TokenArray,Type,Residuum)->
    ...
    parse_tokens(R2,Name,TokenArray,[{absolute_area,{tArea,[NewDetails|{type,Type}],
				     {return,reference}}}
		     |Residuum]);
...
parse_tokens(tMemArea,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{memory_area,{tMemArea,[{value,MemArea},{type,Type}],
				   {return,reference}}}|Residuum]);
...
parse_tokens(tMemErr,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{memory_err,{tMemErr,[{value,Size},{type,Type}],
				  {return,reference}}}|Residuum]);
...
parse_tokens(tMemNoMem,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     Size:16/little-unsigned-integer,
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{memory_no_memory,{tMemNoMem,[{value,Size},{type,Type}],
					{return,reference}}}|Residuum]);
...
parse_tokens(tMemFunc,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     Size:16/little-unsigned-integer,
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{memory_function,{tMemFunc,[{value,Size},{type,Type}],
				       {return,reference}}}|Residuum]);
...
parse_tokens(tAreaErr,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    parse_tokens(R2,Name,TokenArray,[{area_error,{tAreaErr,[{type,Type}],{return,reference}}}
		     |Residuum]);
...
parse_tokens(tRefN,Name,Bin,TokenArray,Type,Residuum)->
    ...
    parse_tokens(R2,Name,TokenArray,[{relative_reference,{tRefN,[{value,ParsedAddy},{type,Type}],
		      {return,reference}}}|Residuum]);
...
parse_tokens(tAreaN,Name,Bin,TokenArray,Type,Residuum)->
    ...
    parse_tokens(R2,Name,TokenArray,[{relative_area,{tAreaN,[NewDetails|{type,Type}],
		      {return,reference}}}|Residuum]);
...
parse_tokens(tNameX,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    %% this is a *ONE* based index so drop it by one...
    parse_tokens(R2,Name,TokenArray,[{name_xref,{tNameX,[{reference_index,RefIndex-1},
...
parse_tokens(tArea3d,Name,Bin,TokenArray,Type,Residuum)->
    <<_Tk:8/little-unsigned-integer,
     ...
     R2/binary>>=Bin,
    {ParsedStart,ParsedEnd}=parse_cell_range(<<Range:64/little-unsigned-integer>>),
    NewDetails=[{start_cell,ParsedStart}|{end_cell,ParsedEnd}],
...
parse_cell_range(Range)->
    ...
    {{FirstRowIdx,FirstCol,FirstRowType,FirstColType},{LastRowIdx,LastCol,LastRowType,LastColType}}.
