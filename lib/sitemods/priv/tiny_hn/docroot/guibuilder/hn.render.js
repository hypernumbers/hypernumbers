      ...
      // mark all bits on the form as being form elements
        ...
        $(self).unbind().bind("submit", function(e) {
            ...
            items.each(function() {
                ...
                path    = HN.Util.relToAbsPath(document.location.pathname, 
                                               ref.path);
                
                values.push({"ref"     : path + HN.Util.refToStr(ref),
                             "formula" : HN.Widgets[type].value(this)});
            });
            
            var submit = function(urm) { 
                console.log(urm);
                HN.Widgets.form.submitted(self); 
            };
            
            $.post(document.location.pathname, 
                   JSON.stringify({"set":{"list": values}}), submit, "json");
    ...
    "submitted": function(self) {
        $(self).empty()
            .append("<p>Thanks for filling out our form! "
                    + "Your response has been entered into our spreadhseet</p>");
    }
};
...
HN.Widgets.radio = {
    ...
    "render": function(self, data) {
        ...
        if( from == null || $(self).children().length > 0) {
            return $(self).clone();
        }
        var ref = HN.Util.parseRef(from),
        read    = function(y, x) {
            ...
            if( this.nodeName == "FORM") {
                ...
                if (!is_in_form && (data_to && (
                    type === "input"
                        ...
                        || type === "radio"
                        || type === "checkbox"))) {
                    $(this).find("input").unbind().bind("blur", function(e) {
                        e.preventDefault();
                        var ref = HN.Util.parseRef($(this).parent()
                                                   .attr("data-binding-to"));
                        var value = {"ref"     : HN.Util.refToStr(ref),
                                     "formula" : e.currentTarget.value};
                        var data = {"set": value};
                        $.post(ref.path + HN.Util.refToStr(ref), 
