HN.Data = function(options)
{
    ...
    {    
        ...
        var py = pages[path].data.cell[y];
        return py && pages[path].data.cell[y][x] || false;
    };

    public.lookupCSS = function(path, y, x) {
        var obj = public.read(path, y, x);
        return obj.style && 
            pages[path].data.styles[obj.style];
    };
    ...
    function updater(time) 
    {
        ...
        var filterRoot = function(path) { 
            return path != document.location.pathname; };        
        var pathstr = $.grep(public.getLoadedUrls(), filterRoot).join(",");

        if( update === null ) {
            
            var args = {"updates":time, "paths":pathstr, "view": tpl}; 
            update = $.get(document.location.pathname, args, fun, "json");
        }
    };
...
HN.PageData.prototype.load_data = function(loaded)
{
    var that = this;
    var path = this.path;
    var args = ( this.path == document.location.pathname )
        ? {"view" : this.tpl}
        : {"via"  : document.location.pathname, "view": this.tpl};
    ...
    $.ajax({
        ...
        "success"  : function(data) {
            ...
            that.set_max();
            HN.Util.dispatch(that.options.dataLoaded, [path]);
            if( loaded != null ) {
                loaded(data.time);
            }
        }
