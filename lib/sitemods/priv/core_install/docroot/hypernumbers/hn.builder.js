...
HN.Builder = function(data, layout)
{
    ...
    public.viewCode = function() {
        stopConfiguring();
        var nodes = $(public.model,"#hnbody")[0].childNodes;
        $("#preview")[0].innerHTML =
            "<textarea id='previewcode'>"
            +render.renderHtml(nodes)
            +"</textarea>";
    };
    ...
    public.saveCurrentView = function(overwrite, callback) {
        ...
        // var $html = $("#preview").children().clone();
        // $html.find("*").removeAttr("data-hnid").removeAttr("style");
        // var html = renderHtml($html[0]);

        var params = {"saveview": {
            "name"      : pathFromDetails(),
            "tpl"       : render.renderHtml(public.model),
            "overwrite" : overwrite
        }};
    ...
    function bindEvents() {
        ...
        $("#required").bind("change", function() {
            console.log(getModelObj(public.model, configuring));
            if( this.checked ) {
        ...
        $(window).bind("unload blur", function(e) { 
            if( state == "RENDER" ) {
                public.saveCurrentView(true, null);
            }
        });
    ...
    function configureElement(id) {
        ...
        console.log($viewObj.attr("data-required"));
