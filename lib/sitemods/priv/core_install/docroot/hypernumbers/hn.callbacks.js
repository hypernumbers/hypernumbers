...
{
    var json = {"borders": {"where": where, "border": border,
...
{
    var right = range.x2 - range.x1;
    var down  = range.y2 - range.y1;
    var post  = {"set":{"merge": {"right":right, "down":down}}};
    HN.Util.postCell(path, {"x":range.x1, "y":range.y1}, post);
};

HN.Callbacks.addUser = function(user, pass, cb) {
    HN.Util.postPath("/_admin/", 
                     {"admin": {"add_user": {"user":user, "pass":pass}}}, 
                     cb);
};