    ...
    public.iterate = function(callback) {
        ...
        for( var y = 0; y < rows; y++ ) {
            ...
            for( var x = 0; x < cols; x++ ) {
                ...
                if( cell && cell.merge ) { 
                    zindex += 1000;
                    for( var z = 0; z < cell.merge.right; z++ ) { 
                        thiswidth += sheet.col_width(tcol+z+1);
                    }

                    for( z = 0; z < cell.merge.down; z++ ) { 
                        thisheight += sheet.row_height(tcol+z+1);
                    }

                }

                var xyz   = "top:"+(totalh-1)+"px;left:"+(totalw-1)+"px;"
                    +"height:"+(thisheight-1)+"px;width:"+(thiswidth-1)+"px;";
                                
                if( !cell || tcol < 0 || trow < 0 ) {
