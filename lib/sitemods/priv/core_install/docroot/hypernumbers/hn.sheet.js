HN.Sheet = function(path, data)
{
    ...
    function processMergedCell(y, x, cell) { 
        ...
        var range = HN.Util.parseAddr(cell.merge);
        var width = 0, height = 0;

        for( var iy = range.obj.y1; iy <= range.obj.y2; iy++ ) {
                        
            width  = 0;
            height += public.row_height(ix);
            
            for( var ix = range.obj.x1; ix <= range.obj.x2; ix++ ) {
    
                width += public.col_width(iy);
