...
HN.Layout.Panes = function(layout, selection, sheet) {
    ...
    public.bounds = {"x1":0, "y1":0, "x2":0, "y2":0};
    ...
    for( var i = 0; i < p[0].length; i++ ) {
        public.bounds.x2 += p[0][i].width;
    }

    public.firstPane = function() {
        return p[0][0];
    };
    ...
    public.refresh = function(args) {

        public.bounds.x2 = public.bounds.x1;
        public.bounds.y2 = public.bounds.y1;
    ...
    public.moveTo = function(topI, leftI) {    
        ...
        drawIndex(Y, true, false, 0, pos.top, sheet.offset.y, sheet.offset.y+cellsy);
        drawIndex(X, true, false, pos.left, 0, sheet.offset.x, sheet.offset.x+cellsx);
    ...
    public.shift_y = function(up) {
        ...
        if( up ) {
            public.bounds.y1 += oldheight;
            public.bounds.y2 += newheight;
        } else {
            public.bounds.y1 -= newheight;
            public.bounds.y2 -= oldheight;
        }
    ...
    public.shift_x = function(left) {
        ...
        if( left ) {
            public.bounds.x1 += oldwidth;
            public.bounds.x2 += newwidth;
        } else {
            public.bounds.x1 -= newwidth;
            public.bounds.x2 -= oldwidth;
        }
