// TODO: Get rid of globals, expose one for use through console.
HN.Site = function() {
...
var initialLoad = function(initPath) {
    ...
    var lang = sheets[initPath].data.data.lang;
...
var showError = function(ind, original) {
    ...
    var cell   = layout.s.cell(ind.y, ind.x);
...
var sheetDataLoader = function(sheetPath) {
    ...
    if( sheetPath == hn.currentPath(HN.url) ) {
        ...
        //console.log(layout.inner.style.top, layout.inner.style.left);
        // Reset to A1, TODO: reset to prev
        // layout.inner.style.top = "0px";
        // layout.inner.style.left = "0px";
        ...
        if( name == "pages" ) {
            ...
            var opts = {
                ...
                "itemChosen" : function() {
                    ...
                    var nurl = ( this.path.length == 1 )
                        ? "/" : "/"+this.path.slice(1).join("/")+"/";
                    
                    window.location.hash = hn.hashUrl.setParam("path", nurl);
                    this.deactivate();
                }
            };
