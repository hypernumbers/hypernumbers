...
HN.Layout.Cell.prototype.end_edit = function(val)
{
    ...
    if( this.selection.is_light_edit() ) {
        ...
        HN.Callbacks.set_cell(this.layout.s.path(), 
...
HN.Layout.Selection.prototype.mouseDownAxis = function(e, xy)
{
    ...
    if( e.target.className == "handle" && e.button != 2 ) {
        ...
        var mouseup = function( e ) {
            ...
            if( xy == Y ) {
                HN.Callbacks.setHeight(that.layout.s.path(), 
                                       index, new_val+4);
            } else {
                HN.Callbacks.setWidth(that.layout.s.path(),
                                      index, new_val+4);
            }
        };
...
{
    var sheet   = this.layout.s;
    var current = this.dom.current.style;
...
HN.Layout.Selection.prototype.do_keydown = function(e)
{
            ...
            && (e.keyCode == HN.Keys.DELETE || e.keyCode == HN.Keys.BACKSPACE))
    {
        ...
        HN.Callbacks.clear(this.layout.s.path(), 
    ...
    if( this.is_editing() ) {
        if( this.c.is_formula() &&
            ...
            select_cell = function() {
                ...
                that.highlightRange(HN.Util.id("dragfill"), tmp);
            };
...
{
    this.state = HN.States.SELECTED_RANGE;
    HN.Callbacks.pasteRange(this.layout.s.path(), this.bounds, 
...
HN.Layout.Selection.prototype.copy = function()
{
    ...
    localStorage.copied = escape(JSON.stringify({
        "bounds" : this.bounds,
        "host"   : this.layout.s.key("host"),
        "path"   : hn.currentPath(),
        "values" : str
    }));

    this.copied = HN.Util.clone(this.bounds);
    this.highlightRange(HN.Util.id("copied"), this.copied);
...
HN.Layout.Selection.prototype.pasteText = function(val)
{
    ...
    if(val.match("\t") === null && val.match("\n") === null) {
        ...
        this.state = HN.States.SELECTED_RANGE;
        HN.Callbacks.pasteValues(this.layout.s.path(), 
                                 this.bounds,lines);
...
HN.Layout.Selection.prototype.dragdown = function(e)
{
    ...
    var mouseup = function(e) {
        ...
        HN.Callbacks.drag(that.layout.s.path(), 
...
HN.Layout.Selection.prototype.show_cell = function() {
    ...
    var sheet     = this.layout.s;
    var current   = this.dom.current.style;
    var cell_attr = sheet.cell(this.cell.y, this.cell.x);
    var cell_pos  = sheet.cell_offset(this.cell.y, this.cell.x);
    var formula   = (cell_attr.formula) || "";
    ...
    current.top    = (cell_pos.top-1)  + "px";
    current.left   = (cell_pos.left-1) + "px";
    current.height = (sheet.row_height(this.cell.y)-1) + "px";
    current.width  = (sheet.col_width(this.cell.x)-1) + "px";
};

HN.Layout.Selection.prototype.show_selection = function()
{
    var bounds = this.order_bounds(this.bounds);
    var coords = this.highlightRange(this.wrapper, bounds);

    var col    = this.dom.colselection;
    var row    = this.dom.rowselection;

    this.dom.name.innerHTML = HN.Util.range_to_str(bounds);
...
HN.Layout.Selection.prototype.highlightRange = function(Dhighlight, range)
{
    var sheet  = this.layout.s;
    var style  = Dhighlight.style;
    var coords = sheet.cell_offset(range.y1, range.x1);
    
    for( var x = range.x1, width = 0; x < range.x2+1; x++ ) {
...
{
    HN.Callbacks.deleteRowCol(this.layout.s.path(), this.bounds, Y);
    return true;
};

HN.Layout.Selection.prototype.menuAction_insertrowsafter = function(e)
{
    HN.Callbacks.insertRowCol(this.layout.s.path(), this.bounds, Y, "after");
    return true;
};

HN.Layout.Selection.prototype.menuAction_insertrowsbefore = function(e)
{
    HN.Callbacks.insertRowCol(this.layout.s.path(), 
...
{
    HN.Callbacks.deleteRowCol(this.layout.s.path(), this.bounds, X);
    return true;
};
...
HN.Layout.Selection.prototype.menuAction_pastelink = function(e)
{
    ...
    HN.Callbacks.pasteValues(this.layout.s.path(), this.bounds, arr);
...
{
    HN.Callbacks.clear(this.layout.s.path(), this.bounds, "all");
    return true;
};

HN.Layout.Selection.prototype.menuAction_clearformat = function(e)
{
    HN.Callbacks.clear(this.layout.s.path(), this.bounds, "style");
    return true;
};

HN.Layout.Selection.prototype.menuAction_clearcontent = function(e)
{
    HN.Callbacks.clear(this.layout.s.path(), this.bounds, "contents");
    return true;
};
