...
HN.Layout.Selection = function(layout, sheet)
{
    ...
    public.scrollIntoView = function(y, x, dir) {
        ...
        if( !tmp.x && (dir == HN.UP || dir == HN.DOWN) ) {
            //panes.moveTo(
            console.log(sheet.offset.y, x);
        } else if ( !tmp.x ) {
            layout.scrollBy1(dir == HN.RIGHT, X);
            public.scrollIntoView(y, x, dir);
        }
        
        if( !tmp.y && (dir == HN.RIGHT || dir == HN.LEFT) ) {
            console.log(y, sheet.offset.x);
        } else if( !tmp.y ) {
