/**
 * @class HN.Layout
 * Rename this to grid / layoutgrid / sheetgrid /grid ui or something
...
HN.Layout.Selection = function(layout, sheet)
{
    ...
    public.gridContextMenu = function(e)
    {
        ...
        var hide = function(e) {
            ...
            if( public.menuItemPicked(target) ) {
    ...
    public.axisContextMenu = function(e, axis)
    {
        ...
        var hide = function(e) {
            if( public.menuItemPicked(e.target) ) {
                HN.Util.removeEvent(document, "mousedown", hide);
                el.style.display = "none";
            }
        };
    ...
    public.is_viewable = function(y, x)
    {
        ...
        var left  = Math.abs(layout.gridPos.top());
        
        if( coord.top < top || (coord.top+sheet.row_height(y)) 
            > top + layout.gridSizePix().y ) {
            ret.y = false;
        }

        if( coord.left < left || (coord.left+sheet.col_width(x)) 
    ...
    {
        if( public.state == HN.States.NOT_EDITING
            || public.state == HN.States.COPY_URL ) {
            return;
        }
    ...
    public.dragdown = function(e)
    {
        ...
        var auto_scroll = function() {
            layout.crollBy1(auto.args.forward, auto.args.axis);
    ...
    {
        if( public.state == HN.States.NOT_EDITING || !public.canEdit() ) {
            return;
        }
    ...
    public.mousedown = function(e) {
        ...
        if( $(e.target).parents("#grid").length == 0 && 
            ...
            public.state = HN.States.NOT_EDITING;
            return;
        } else if ( public.state == HN.States.NOT_EDITING ) {
            public.resumeSelection();
        }
    ...
    {
        HN.UIActions.open_dialog(layout, "import");
        return true;
    };
    ...
    {
        HN.UIActions.open_dialog(layout, "mark");
        return true;
    };
...
HN.Layout.Cell = function(selection, sheet)
{
    ...
    public.calculateWidth = function() {
        ...
        if( input.style.whiteSpace == "normal" ) {
            var xy = Y;
            var t  = (sheet.col_width(selection.cell.x)-2) +"px";
