 ...
 */
HN.Layout = function(sheet /*sheet*/) 
{
    var public = {};
    public.s = sheet;
    ...
    function calcScrollbarPos(xy)
    {
        ...
        var gutter_s = size[xy.str] - bar_s;
        var pane_pos = -parseInt(inner.style[xy.coord], 10);
        var pos      = parseInt(gutter_s * pane_pos / public.s[xy.dimension]);
    ...
    function calcScrollbarSize(xy)
    {
        ...
        var bar_size    = (size[xy.str] / public.s[xy.dimension]) * gutter_size;
    ...
    function dragBoxPressed(clk_event, xy)
    {
        ...
        function scroll(mve_event) {
            ...
            var ngridpos = ((public.s[xy.dimension] - size[xy.str]) / 100) 
                * percentage_scrolled;
    ...
    function gutterPressed(event, xy) 
    {
        ...
        function gutterScroll() {
            ...
            var bar_size   = parseInt(scrollbar[xy.str].style[xy.dimension],10);
    ...
    {
        if (!forward && public.s.offset[xy.str] == 1) 
            return;
        
        var f = ( xy == Y ) ? "row_height" : "col_width";
        if (forward)
            doScroll(1, -public.s[f](public.s.offset[xy.str]), xy, true);
        else
            doScroll(-1, public.s[f](public.s.offset[xy.str]-1), xy, true);
    };
    ...
    function snapToDistance(forward, distance, xy)
    {
        ...
        while( true ) 
        {
            ...
            if( total > distance) {
                total -= public.s[f](i);
                return { 
                    "offset" : i - public.s.offset[xy.str] + (forward ? 0 : 1), 
                    "dist"   : -step * total, 
                };
            } 
            ...
            if (i < 1) {
                return { 
                    "offset" : 1 - public.s.offset[xy.str], 
