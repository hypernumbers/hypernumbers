...
HN.ToolBar = function(layout)
{
    ...
    HN.Util.addEvent(HN.Util.id("submitmark"), "mousedown", function(e) {
        HN.UI.close_dialog(that.layout, "mark");
    ...
    HN.Util.addEvent(HN.Util.id("rtebtn"), "mousedown", function(e) {
        $("#wmd-preview").val("");
        $("#wmd-input").val(that.layout.tabUI().currentCellValue());
    ...
    HN.Util.addEvent(HN.Util.id("borders"), "mousedown", function(e) {
        if( e.target.nodeName== "A" && 
            ...
            var bounds = this.layout.tabUI().currentSelectedBounds();

            HN.Callbacks.setBorders(this.layout.currentSheet().path(),
                                    bounds, where, border,
