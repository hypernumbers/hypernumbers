...
HN.Builder = function(data, layout)
{
    ...
    function bindEvents() {
        ...
        $("#createviewbtn").click( function() {
            ...
            if( tpl == "blank" ) { 
                public.loadHtml("<div class='droppable'></div>");
            } else {
                public.loadTplFile(path, "/templates/"+tpl+"/index.tpl");
            }

            public.setDetails(path, { 
    ...
    function configureElement(id) {
        ...
        $("#bindfrom").val(viewObj[0].getAttribute("data-binding-from"));
        $("#bindto").val(viewObj[0].getAttribute("data-binding-to"));
        $("#changetype").val(viewObj[0].getAttribute("data-type"));
    ...
    function applyCSS($dom, css) {
        for( var i=0, len=css.length; i<len; i++ ) {
            ...
            if( rule.selector.match(":") ) {
                ...
                $dom.find(rule.selector).attr("style", rule.styles);
            }
        }
    };
