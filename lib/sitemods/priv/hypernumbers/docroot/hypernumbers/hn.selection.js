...
HN.Layout.Selection.prototype.mouseDownAxis = function(e, xy)
{
    ...
    if( e.target.className == "handle" && e.button != 2 ) {
        ...
        offset   = (xy == Y) 
            ? HN.Util.y_pos($("#scroller")[0]) + tmp
            : HN.Util.x_pos($("#scroller")[0]) + tmp,
...
{
    return $.inArray("write", this.layout.s.data.data.permissions) != -1;
};
...
{
    $("#viewbuilderintro").show();
    $("#mainviewbuilder").hide();
    this.layout.state = layout.states.MOST_VIEWBUILDER;
    this.layout.gridResize();
};