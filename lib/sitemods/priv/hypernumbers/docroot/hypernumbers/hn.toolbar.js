...
HN.ToolBar = function(layout)
{
    ...
    HN.Util.addEvent(HN.Util.id("markbtn"), "mousedown", function(e) {
        HN.UIActions.open_dialog(that.layout, "mark");
    });
    ...
    HN.Util.addEvent(HN.Util.id("guibuilder"), "mousedown", function(e) {
        document.location.href = document.location.pathname
            +"?view=_global/pagebuilder";
    });

    HN.Util.addEvent(HN.Util.id("rte_submit"), "click", function(e) {
        var val = $("#wmd-input").val();
        HN.Callbacks.set_cell(that.layout.s.data.data.path,
                               that.layout.selection.cell, val);
        HN.Callbacks.format(that.layout.s.data.data.path,
    ...
    HN.Util.addEvent(HN.Util.id("borders"), "mousedown", function(e) {
        if( e.target.nodeName== "A"
            ...
            HN.Callbacks.setBorders(that.layout.s.data.data.path,
    ...
    var opts = {
        callback : function(el) {
            return that.layout.selection.menuItemPicked(el);
        }
    };
    $("#menu").filemenu(opts);
};
...
HN.ToolBar.prototype.newPage = function()
{
    ...
    var do_new = function(name) {
        ...
        if( arr.length == 0 ) {
            $("#newpageerror").html("<strong>Error : </strong> The "
                                    + "name can only contain letters,"
                                    + "<br />numbers or the characters  \"_\" or \"-\"");
        } else {
            var url = "/u/"+that.layout.s.data.data.user+"/"+arr.join("/")+"/";
            window.open(url);
            HN.Util.id("newpagename").value = "";
            HN.UIActions.close_dialog(that.layout, "newpage");
        }
    };
...
HN.ToolBar.prototype.setFormat = function(value)
{
    ...
    HN.Callbacks.format(layout.s.data.data.path, sel.bounds, format);
};
...
HN.ToolBar.prototype.setStyle = function(style, value)
{
    ...
    HN.Callbacks.style(sel.layout.s.data.data.path, sel.bounds, style, value);
};
