...
%%                [#item{attrs=Attrs}] ->
%%                    [{lobj_to_ref(S, LO), binary_to_term(Attrs)} | Acc];
%%                []                   ->
%%                    Acc end,
...
%%     #refX{site = Site, type = Ty, path = binary_to_term(P), obj = O}.
...
%%                  end
%%          end,
%%     XRefXs = expand_ref(Ref),
%%     [apply_to_attrs(X, Op(X), Action, Uid) || X <- XRefXs],
%%     % now mark the refs dirty for zinfs
%%     [ok = mark_dirty_for_zinf(X) || X <- XRefXs],
%%     ok = mark_these_dirty(XRefXs, Uid),
%%     ok.
...
%%                  #refX{site = S, path = binary_to_term(P), type = Ty, obj = O};
