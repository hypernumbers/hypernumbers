    ...
    "submitted": function(self) {
        $(self).empty()
            .append("<h2>Thanks for filling out our form!</h2>"
                    ...
                    + "<p>Head over to <a href=\"http://tiny.hn\">tiny.hn</a> to"
                    + "get your own tiny site and build your own forms. It's free"
                    + "and it takes less than a minute</p>");
    }
};
    ...
    public.render = function(doc) {
        $(doc).find(".hn").each( function() {
            if( this.nodeName == "FORM") {
                ...
                console.log($(this).parent().attr('data-binding-to'));
                var ref = HN.Util.parseRef($(this).parent().attr("data-binding-to"));
                var value = {"ref"     : HN.Util.refToStr(ref),
                             "formula" : e.currentTarget.value};
                var data = {"set": value};
                console.log(data);
                $.post(ref.path + HN.Util.refToStr(ref), JSON.stringify(data), "json");
                return false;
                });
            } else {
                $(this);
