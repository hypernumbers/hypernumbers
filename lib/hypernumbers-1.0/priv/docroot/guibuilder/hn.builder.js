...
HN.Builder.prototype.load_views = function()
{
    ...
    $.getJSON("/?templates", function(data) {
        $.each(data, function() {
            $("#templateslist").append("<li name='"+this+"' class='btn'>"
                                       +this+"</li>");
        });
    });
...
HN.Builder.prototype.setup_events = function()
{
    ...
    $saveview.click( function() {
        ...
        var user = readCookie("auth").split(":")[1],
...
var sheet, layout, builder, data, path = document.location.pathname;
var options = {
    
    dataLoaded: function() {
        sheet   = new HN.Sheet(data.pages[path]);
        builder = new HN.Builder(sheet);
        layout  = new HN.Layout(sheet);

        if( $.inArray("admin", data.pages[path].data.groups) ) {
            $(".global").show();
        }
