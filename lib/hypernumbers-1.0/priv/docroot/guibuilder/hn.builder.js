...
HN.Builder = function(data, layout)
{
    ...
    function setupEvents() {
        ...
        $saveview.click( function() {
            ...
            var params = {"saveview": {
                "name"   : $("#groupslist").val() + $("#viewname").val(),
                "tpl"    : renderHtml(public.model)
            }};

            var url  = $("#groupslist").val() + $("#viewname").val(),
        ...
        $("#views").click( function(e) {
            ...
            if( e.target.nodeName == "LI" ) {
                ...
                loadHtml(document.location.pathname+"?view="+name+"&template");

                $("#views").toggle("medium");
                $("#groupslist").val(split[0]+"/"+split[1]+"/");
                $("#viewname").val(split[2]);
            }
        });
        ...
        $("#templates").click( function(e) {
            if( e.target.nodeName == "LI" ) {
                var name = e.target.getAttribute("name");
                loadHtml("/templates/"+name+"/index.tpl");
                selectTab("add");
            }
        });
    ...
    var options = {
        ...
        dataLoaded: function() {
            ...
            if( !layout ) {
                ...
                $("#groupslist").append("<option value='_u/"
                                        +data.getPage(path).user
                                        +"/'>Mine</option>");

                $.each(data.getPage(path).groups, function(i, name) {
                    $("#groupslist").append(
                        "<option value='_g/"+name+"/'>"+name+"</option>");
                });
