...
HN.Widgets.form.bindFrom =
    function(self, path, range) {
        ...
        for( var x = range.x1; x <= range.x2; x++ ) {
            ...
            var to = path +
                HN.Util.refToStr({"type":"col_range", "obj":{"x1":x}});
            var from = path +
        ...
        $("#use_range").click( function() {
            if( configuring ) {
                ...
                if( typeof HN.Widgets[type][toOrFrom] != "undefined" ) {
                    HN.Widgets[type][toOrFrom](obj, layout.s.data.path,
                                               bounds);
                } else {
                    var attr = ( toOrFrom == "bindTo" )
                        ? "data-binding-to" : "data-binding-from";
                    var ref = HN.Util.range_to_str2(bounds);
                    var str = layout.s.data.path + ref;
                    var rel = $("#togglepathstyle");
                    if (rel[0].checked) {
                      var refx = HN.Util.parse_path_and_ref(str);
                      var path = HN.Util.make_relative(document.location.pathname, refx.path);
                      obj.setAttribute(attr, path + refx.ref);
                    } else {
