...
HN.Builder = function(data)
{
    ...
    this.htmlRender = new HN.HTMLRender(data);
    //    this.layout = layout;
    //    this.sheets  = sheets;
...
HN.Builder.prototype.load_html = function(name, path)
{
    var that = this;
...
HN.Builder.prototype.configure_element = function(obj)
{
    ...
    HN.Builder.Widgets[obj.getAttribute("data-type")].configure(this.layout, this.sheets, obj);
...
HN.Builder.prototype.setup_events = function()
{
    ...
    $("#views").click( function(e) {
        if( e.target.nodeName == "LI" ) {
            ...
            split    = name.split("/");   
            that.load_html(name, "/views/"+name+".tpl");
            $("#views").toggle("medium");
    ...
    $("#templates").click( function(e) {
        if( e.target.nodeName == "LI" ) {
            var name = e.target.getAttribute("name"); 
            that.load_html(name, "/templates/"+name+"/index.tpl");
        }
    });
...
{
    var toRender = $(this.model).clone();
    //var view = this.render_model(toRender, "preview");
    $(this.dom.preview).empty().append(toRender);
    if( this.configuring && this.state == HN.Builder.EDITING ) {
        var id = this.configuring.attr("data-hnid");
        toRender.find("[data-hnid="+id+"]").addClass("configuring");
    }
};
...
HN.Builder.prototype.render_model = function(obj, rendertype)
{
    ...
    //     return HN.Builder.Widgets[obj.getAttribute("data-type")].render(
    //         this.layout, this.sheets, obj);
    // } else {
    //     for(var i = 0; i < obj.childNodes.length; i++) {
...
HN.Builder.Widgets.text = {
    ...
    "configure": function(layout, sheets, self) {
    ...
    "render": function(layout, sheets, self) {
        ...
        if( from === null ) {
            ...
            text = sheets[document.location.pathname]
                .cell(ref.y, ref.x).value || "Blank Cell";
        }
...
HN.BuilderBoot = function() 
{
    ...
    var load_sheet = function(url) {
        ...
        if( typeof sheet[path] !== "undefined" ) {
            layout.sheet = sheets[path];
    ...
    var options = {
        ...
        dataLoaded: function() {
            ...
            sheets[path] = new HN.Sheet(data);
            
            if( !layout ) {
        
                layout  = new HN.Layout(sheets[path]);
                console.log([layout, sheets]);
                builder = new HN.Builder(data);
        ...
        pagesLoaded: function(data) {
            var opts = {
                "visitHandler" : function(url) { 
                    load_sheet(url);
                    pagemenu.close();
                }
            };
            
            pagemenu = new HN.PageMenu(opts);
            pagemenu.populate(path, data);            
        },
    ...
    data.loadPagesMenu(document.location.pathname);
    data.addPage(path);
