...
HN.Gui.Register = function(type)
{
  ...
  for (r in registerobj) {
    pagesobj[registerobj[r].to_page] = {"page": "not loaded yet",
                                            "bindings": []};
...
HN.Gui.Register.parse_element = function(element) {
  ...
  register['to_page'] = bindingsobj['page'];
  register['to_ref']  = bindingsobj['ref'];
  register['to_type'] = bindingsobj['type'];
  sourceobj = HN.Gui.Register.parse_binding(source);
  register['from_page']  = sourceobj['page'];
  register['from_ref']   = sourceobj['ref'];
  register['from_type']  = sourceobj['type'];
  ...
  if (register['componenttype'] === "block"
    ...
    if ((register['to_type'] === "cell")
      || (register['to_type'] === "col")
      || (register['to_type'] === "col_from_last")
      || (register['to_type'] === "row")
      || (register['to_type'] === "row_from_last")) {
  ...
  // col_range_from_last or row_range_from_last
    ...
    if ((register['to_type'] === "cell")
      || (register['to_type'] === "range")
      || (register['to_type'] === "col_range_from_last")
      || (register['to_type'] === "row_range_from_last")) {
          register['validity']    = "valid";
    } else {
          register['validity']    = "invalid";
    };
...
HN.Gui.Register.form_submit = function(id) {
  ...
  for (binding in bindings) {
    ref = HN.Gui.Register.parse_path_and_ref(bindings[binding].url);
    if (ref.path[0] === "/") {
      rel_path = HN.Gui.Register.make_relative(ref.path);
    } else if (ref.path[0] === ".") {
      rel_path = ref.path;
    }
    if (!pageobj[rel_path]) {
      ...
      pageobj[rel_path][ref.ref] = bindings[binding].val;
  };
  for (page in pageobj) {
    list = [];
