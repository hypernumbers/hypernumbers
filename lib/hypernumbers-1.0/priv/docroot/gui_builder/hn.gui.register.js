...
HN.Gui.Register = function(type)
{
  ...
  var options = new HN.Gui.DataOptions("hey", "ho");
  for (p in pagesobj) {
    console.log(p);
    pagesobj[p].page = new HN.Data(p, options);
  };
  this.pages = pagesobj;
...
HN.Gui.Register.parse_element = function(element)
  {
    ...
    // takes a source of values (eg a drop down list, a radio box or a check box
    ...
    case 4:
      register['type'] = bits[2];
      bindingsobj = HN.Gui.Register.page_from_binding(bits[3]);
      register['bindingpage'] = bindingsobj['page'];
      register['bindingref'] = bindingsobj['ref'];
      // the source is the same as the binding
    ...
    case 5:
      register['type'] = bits[2];
      bindingsobj = HN.Gui.Register.page_from_binding(bits[3]);
      sourceobj = HN.Gui.Register.page_from_binding(bits[4]);
      register['bindingpage'] = bindingsobj['page'];
      register['bindingref'] = bindingsobj['ref'];
...
HN.Gui.Register.page_from_binding = function(binding)
{
  var bits;
  bits = binding.split("/");
    switch (bits[0]) {
    case "http:":
      return HN.Gui.Register.make_url(binding);
    case "https:":
      return HN.Gui.Register.make_url(binding);
    default:
      switch (binding[0]) {
        case "/":
          return HN.Gui.Register.make_url(document.location.protocol + "//"
...
HN.Gui.Register.make_url = function(string)
{
  var bits;
  var returnobj = {};
  bits = string.split("/");
