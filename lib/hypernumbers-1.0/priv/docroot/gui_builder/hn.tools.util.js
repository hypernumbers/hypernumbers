...
HN.Tools.Util.new_page = function() {
  ...
  console.log("full_path is " + new_full_path);
  if (new_full_path.checked) {
    // a full path should start with a '/' - if not make it so
    console.log("full starts with " + value[0]);
    if (value[0] === "/") {
...
HN.Tools.Util.configure_paths = function() {
  console.log("in configure_paths...");
  var pageelem;
  var bindings_page;
  ...
  fn = function(ev) {
    ...
    if ((ev.target.selectedIndex + 1) === ev.target.options.length) {
      ...
      console.log("redrawing bindings box...");
      ...
      console.log("current page is " + Register.currentpage);
      console.log("current range is " + Register.currentrange);
