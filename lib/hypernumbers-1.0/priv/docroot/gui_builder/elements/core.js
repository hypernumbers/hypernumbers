...
HN.Tools.Elements.Core.make_lable = function() {
  ...
  elem.innerHTML = "Unconfigured Lable <em>(Click To Configure)</em>";
  return elem;
};
...
HN.Tools.Elements.Core.prototype.configure1 = function()
{
  ...
  $("#bindings_full_path").bind("change", core.configure_paths);
...
HN.Tools.Elements.Core.configure_paths = function() {
  var pageelem;
  var bindings_page;
  ...
  for (page in HNGui.register.pages) {
    if (typeof page !== 'function') {
      ...
      if (bindings_full_path.checked) {
        ...
        html += "<option value=\"" + path2 + "\">" + path2a + "</option>";
      }
    }
  }
...
HN.Tools.Elements.Core.configure = function() {
  ...
  HN.Tools.Elements.Core.configure_paths();
