...
HN.ToolBar = function(layout)
{
    ...
    HN.Util.addEvent(HN.Util.id("rte_submit"), "click", function(e) {
        ...
        HN.Callbacks.set_cell(that.layout.s.data.path,
                               that.layout.selection.cell, val);
        HN.Callbacks.format(that.layout.s.data.path,
    ...
    HN.Util.addEvent(HN.Util.id("borders"), "mousedown", function(e) {
        if( e.target.nodeName== "A"
            ...
            HN.Callbacks.setBorders(that.layout.s.data.path,
...
HN.ToolBar.prototype.newPage = function()
{
    ...
    var do_new = function(name) {
        ...
        if( arr.length == 0 ) {
            ...
            var url = "/u/"+that.layout.s.data.user+"/"+arr.join("/")+"/";
            window.open(url);
...
HN.ToolBar.prototype.setFormat = function(value)
{
    ...
    HN.Callbacks.format(layout.s.data.path, sel.bounds, format);
};
...
HN.ToolBar.prototype.setStyle = function(style, value)
{
    ...
    HN.Callbacks.style(sel.layout.s.data.path, sel.bounds, style, value);
};
