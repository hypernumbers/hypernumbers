...
HN.Sheet.prototype.load_data = function()
{
  ...
  var fun = function(data)
  {
    ...
    if( that.spreadsheet.layout ) {
      ...
      that.spreadsheet.layout.state = HN.States.SELECTED_CELL;
    }
    $("#path").text(document.location.pathname);
    document.title = document.location.pathname + " - hypernumbers";
    that.updater(data.time);

    if( that.data["viewed-tour"] === "false" ) {
      $("#loading").fadeOut("slow");
      that.spreadsheet.layout.toolbar.run_intro();
    } else {
...
HN.Sheet.prototype.calc_height = function(row)
{
  ...
  for (var i in obj) {
    if( i.substring(0,10) == "autoheight" && obj[i] == "auto") {
      var col = i.substring(11);
      var hidden    = HN.Util.id("hidden_input");
      var width = this.col_width(col);
  ...
  this.data.row[row][row].aheight = height;
  return height;
};
...
HN.Sheet.prototype.change_row = function(msg)
{
  ...
  if(msg.name.substring(0,10) == "autoheight") {
    this.calc_height(msg.ref);
  }
};
...
HN.Sheet.prototype.delete_row = function(msg)
{
  this.data.row[msg.ref][msg.ref][msg.name] = null;

  if(msg.name.substring(0,10) == "autoheight") {
    delete this.data.row[msg.ref][msg.ref].aheight;
    this.calc_height(msg.ref);
  }
};
  ...
  var loop = function(time) {
    $.getJSON("?updates="+time, function(data) {
      try {
        ...
        HN.Util.trail(JSON.stringify(err));
        that.spreadsheet.layout.toolbar.dialog_open(HN.Util.id("jserror"));
      }
    });
  };
  loop(time);
