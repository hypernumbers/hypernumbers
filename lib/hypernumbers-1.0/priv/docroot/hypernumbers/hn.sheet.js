...
{
    for( var i in this.data.data.cell ) {
        if( parseInt(i) > this.max.y ) {
            this.max.y = parseInt(i);
        }
        for ( var x in this.data.data.cell[i] ) {
...
{
    if( this.data.data.row[row] && this.data.data.row[row][row] &&
        this.data.data.row[row][row].height ) {
        return this.data.data.row[row][row].height;
    }
    
    var height = HN.Sheet.CELL_HEIGHT;

    for (var i in this.data.data.cell[row]) {
        ...
        var style  = (cell && cell.style 
                      && this.data.data.styles[cell.style]) || "";
    ...
    if( height != HN.Sheet.CELL_HEIGHT) {
        this.data.set("row", row, "aheight", height);
    } else {
        this.data.remove("row", row, "aheight");
    }
...
HN.Sheet.prototype.calc_size = function()
{
    ...
    for (var i in this.data.data.row) {
        if (String(i >>> 0) == i && i >>> 0 != 0xffffffff) {
            rows[i] = true;
        }
    }
    
    for( var s in this.data.data.styles ) {
        if( this.data.data.styles[s].match(/white-space:normal/) ) {
            dirty.push(s);
        }
    };
    
    for( var ix in this.data.data.cell ) {
        for ( var iy in this.data.data.cell[ix] ) {
            if( jQuery.inArray(this.data.data.cell[ix][iy].style+"", dirty) 
                != -1) {
                rows[ix] = true;
            }
        }
    ...
    var width = 0, c = 0;
    for (var x in this.data.data.column) {
        if (String(x >>> 0) == x && x >>> 0 != 0xffffffff) {
            c++;
            width += this.data.data.column[x][x].width;
        }
    }
...
HN.Sheet.prototype.get_style_by_cell = function(cell)
{
    ...
    if(c) {
        return this.data.data.styles[c.style] || "";
    } else {
        return "";
    }
};
...
{
    if( !this.data.data.row[i] || !this.data.data.row[i][i] ) {
        return HN.Sheet.CELL_HEIGHT;
    }
    
    if( this.data.data.row[i][i].height ) {
        return this.data.data.row[i][i].height;
    }

    if( this.data.data.row[i][i].aheight ) {
        var tmp = this.data.data.row[i][i].aheight;
...
{
  return this.data.data.column[i] && this.data.data.column[i][i]
    && this.data.data.column[i][i].width || HN.Sheet.CELL_WIDTH;
};
...
HN.Sheet.prototype.cell = function(y, x)
{
    if( x < 0 ) {
        for (var nx = this.max.y; ny > 0; ny--) {
            if( this.data.data.cell[ny] && this.data.data.cell[ny][x] 
                && this.data.data.cell[ny][x].value ) {
                if( x == -1 ) {
                    return this.data.data.cell[ny][x];
                } else {
                    x += 1;
                }
            }
    ...
    return this.data.data.cell[y] && this.data.data.cell[y][x] || false;
};
