...
HN.Util.postColumn = function(column, json)
{
  var col = HN.Util.to_b26(column);
  var url = document.location.pathname + col + ":" + col +"?attr";
  $.post(url, JSON.stringify(json));
};

HN.Util.postRow = function(row, json)
{
  var url = document.location.pathname + row + ":" + row +"?attr";
  $.post(url, JSON.stringify(json));
};
