var sheet, layout, toolbar, pagemenu,
...
var checkloaded = function() {
    ...
    if( loaded.pages && loaded.data ) {
        pagemenu.populate(path, data.pages[path].pages);

        var read = data.pages[path].data.permissions == "read"
            ? "[readonly] " : "";

        $("#permissions").text(read);
        document.title 
            = read + document.location.pathname + " - hypernumbers";

        if( data.pages[path].data["viewed-tour"] === "false" ) {
...
var options = {
    ...
    dataLoaded: function() {
        ...
        toolbar.user_navigation(data.pages[path].data.user);
        loaded.data = true;
