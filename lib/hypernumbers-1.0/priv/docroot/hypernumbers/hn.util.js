...
HN.Util.parseRef = function(ref) { 
    ...
    range     = list.pop(),
    cells     = range.split(":"),
    start     = HN.Util.parse_ref(cells[0]),
    end       = (cells.length > 1) ? HN.Util.parse_ref(cells[1]) : start,
    type      = start == end ? "cell" : "range";

    if( cells.length == 1 ) {
        range = {"x": start.x, "y": start.y}; 
    } else { 
        range = {"x1": start.x, "y1": start.y, "x2": end.x, "y2": end.y};
    }
