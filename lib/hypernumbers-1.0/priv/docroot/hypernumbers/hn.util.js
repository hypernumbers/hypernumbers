...
HN.Util.parseRef = function(ref) {
    ...
    list.push("");
    if (addr === undefined) {
        return { "path" : HN.Util.relToAbsPath(document.location.pathname, 
                                               list.join("/")),
                 "type" : undefined,
                 "obj"  : undefined };
    } else {
...
HN.Util.isCongruent = function(type1, ref1, type2, ref2) {
  ...
  if (type1 == "cell"
    ...
    if (type2 === "range"
      ...
      range1 = HN.Util.parse_range(ref1.toUpperCase());
      range2 = HN.Util.parse_range(ref2.toUpperCase());
...
HN.Util.parseAddr = function(ref) {
  ...
  if (cell.exec(ref)           && (cell.exec(ref)[0]    === ref)) {
    ...
    var tmp = ref.split(":");
    return {"type":"col_from_last", "obj": {"x1": HN.Util.from_b26(tmp[0]),
    "x2": HN.Util.from_b26(tmp[1])}};
...
HN.Util.parse_ref = function(ref) {
    ...
    if (xref[0] === "-") {
        var x = - HN.Util.from_b26(xref.slice(1, xref.length));
    } else {
        var x = HN.Util.from_b26(xref);
    }
    return {
        "x": x,
    ...
    if (currentpage === page) {
        ...
        for (var i = 0; i <= p_len; i += 1) {
            if (!c_bits[i]) {
                ...
                ret_bits = ret_bits.slice(1, p_bits.length); // discard first element
            } else if (c_bits[i] !== p_bits[i]) {
                return HN.Util.repeat_string("../", p_len - i) + ret_bits.join("/") + "/";
            }
        }
        return "../" + ret_bits.join("/") + "/";
    }
