...
HN.Layout.prototype.scrollButtonPressed = function(event, forward, xy)
{
    ...
    timer = setInterval(function() {that.scrollBy1(forward, xy);}, 100);
...
HN.Layout.prototype.doScroll = function(offset, dist, xy)
{
    ...
    if (this.calcScrollbarPos(xy) >= this.scrollmax[xy.str]) {
        this.s.extend(xy, 2);
    ...
    this.check_pane_bounds(dist, xy, dist <= 0);
}
