...
HN.ToolBar.prototype.addPages = function()
{
  ...
  var add = function(obj, path) {
    var html = "<ul>";
    for (var x in obj) {
      var p = path+x+"/";
      html += "<li><a href=\""+p+"\">"+x+"</a>";
      html += (isEmpty(obj[x])) ? "" : add(obj[x], p);
      html += "</li>";
    }
    return html+"</ul>";
  };
  ...
  $.getJSON("?pages", function(data) {
    ...
    if( path !== "/" ) {
      data = data[path.split("/")[1]];
      path = "/"+path.split("/")[1]+"/";
    }

    var html = "<a>My Spreadsheets</a>";
    if( isEmpty(data) ) {
      html += "<ul><li><a class=\"disabled\"><em>empty</em></a></li></ul>";
    } else {
      html += add(data, path);
    }
    HN.Util.id("browse").innerHTML = html;
