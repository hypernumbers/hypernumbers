...
HN.Layout = function(s /*sheet*/) 
{
    ...
    function dragBoxPressed(clk_event, xy)
    {
        ...
        spos = parseInt(scrollbar[xy.str].style[xy.coord],10),
        attr = (xy == Y) ? "clientY" : "clientX",
        origin = prev = clk_event[attr],
        track_s = size[xy.str] - 32;
        
        function scroll(mve_event) {
            var delta = mve_event[attr] - origin;
            est_dist = s[xy.dimension] * delta / track_s;
            if ((spos + est_dist) > scrollmax[xy.str])
                est_dist = scrollmax[xy.str] - spos;
            var res = snapToDistance(delta > 0, Math.abs(est_dist), xy);
            if (res.dist) {
                // snapped to a new cell
                spos = doScroll(res.offset, res.dist, xy, false);  
                origin = mve_event[attr];
            } else {
                // temporarily move slider
                spos = doScrollbarPos(spos + (mve_event[attr]-prev), xy);
            }
            prev = mve_event[attr];
        };
