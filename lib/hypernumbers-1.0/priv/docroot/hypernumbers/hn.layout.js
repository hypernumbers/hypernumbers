...
HN.Layout.prototype.windowResize = function()
{
  ...
  // TODO : make nice
  this.scrollBy1(true,  Y);
  this.scrollBy1(false, Y);
  this.scrollBy1(true,  X);
  this.scrollBy1(false, X);
...
HN.Layout.prototype.dragBoxMoved = function(position, xy)
{
  ...
  // Difference between current and new position
        diff = -(parseInt(this.inner.style[xy.coord], 10) + actual.position);
...
HN.Layout.Panes.prototype.refresh = function()
{
  ...
  var row = p.row;
  var col = p.col;
  var top = parseInt(p.div.style.top, 10);
  var left = parseInt(p.div.style.left, 10);
...
HN.Layout.Selection.prototype.calculateInputWidth = function()
{
  ...
  var width = hidden.offsetWidth+5;
   tmpwidth = parseInt(input.style.width, 10)-2;

  while( width > tmpwidth ) {
    tmpwidth += sheet.col_width(this.cell.x + this.input_width);
    this.input.style.width = tmpwidth+ "px";
    this.input_width++;
  };
};
...
HN.Layout.Selection.prototype.set_cell = function(y, x)
{
  this.cell = {"y":y, "x":x};
...
HN.Layout.Selection.prototype.select_next = function(key, event)
{
  ...
  {
    if( cell.y < b.y2 ) {
      this.set_cell(cell.y+1, cell.x);
    } else if( cell.y == b.y2 && cell.x == b.x2 ) {
      this.set_cell(b.y1, b.x1);
    } else {
      this.set_cell(b.y1, cell.x+1);
    }
  }
  else if( key == HN.Keys.TAB )
  {
    ...
    if( cell.x < b.x2 ) {
      this.set_cell(cell.y, cell.x+1);
    } else if( cell.y == b.y2 && cell.x == b.x2 ) {
      this.set_cell(b.y1, b.y2);
    } else {
      this.set_cell(cell.y+1, b.x1);
    }
  }
