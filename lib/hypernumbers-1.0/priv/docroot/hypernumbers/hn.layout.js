...
HN.ToolBar.prototype.loadFunctions = function(e)
{
  ...
  // TODO : Fix internationalisation
  $.getJSON("/hypernumbers/fns_en_gb.json", function(data) {
    that.functions = data;
...
HN.ToolBar.prototype.addStatus = function()
{
  function isEmpty(ob) {
    for( var i in ob ){
      if(ob.hasOwnProperty(i)) {
    ...
    for (var x in obj) 
      {
        console.log("x");
        console.log(x);
        var html = "<li><a> " + x + "</a><ul>" ;
        for (var y in obj[x]) 
          {
            console.log("y");
            console.log(y);
            html += "<li><a href=\"" + y + "\">" + y + "</a></li>"
              }
      }
    //html += !top ? "</ul>" : "";
    html += "</ul></li>"
  ...
  $.getJSON("?status", function(data) {
    ...
    if( !isEmpty(data) ) {
      var empty = HN.Util.id("emptystatus");
      empty.innerHTML = add(data, "/", true);
    }
