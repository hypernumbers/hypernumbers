...
HN.Layout = function(parent,name,sheet)
{
  ...
  $("#pathpaste, #hnpaste").click(function() {
...
{
  var input = this.input, cell = this.cell, dom = this.dom,
     layout = this.layout,
      sheet = layout.s,
          s = input.style;
  ...
  window.setTimeout(function() {
    ...
    dom.path_paste.innerHTML = document.location.pathname+tmp;
  },0);
...
HN.Layout.Selection.prototype.mousedown = function(e)
{
  ...
  if( this.last_click && new Date().getTime() - 250 < this.last_click ) {
    this.deselect();
    //console.log(sheet.data.cell[this.cell.y] && sheet.data.cell[this.cell.y][this.cell.x]);
    //console.log([sheet.cell(this.cell.y, this.cell.y)]);
    return;
    this.input.value = sheet.cell(this.cell.y, this.cell.y).formula || "";
    this.input.focus();
