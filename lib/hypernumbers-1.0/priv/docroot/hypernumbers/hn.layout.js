...
{
  HN.Util.postColumn(column, {set: {width: width}});
};

HN.Callbacks.setHeight = function(row, height)
{
  HN.Util.postRow(row, {set: {height: height}});
};
...
HN.Layout.Selection = function(layout)
{
  ...
  HN.Util.addEvent(HN.Util.id("columns"), "mousedown", function(e) {
    that.mouseDownAxis(e, X);
  });

  HN.Util.addEvent(HN.Util.id("rows"), "mousedown", function(e) {
...
HN.Layout.Selection.prototype.mouseDownAxis = function(e, xy)
{
  if( e.target.className == "handle" ) {
    ...
    var mouseup = function( e ) {
      ...
      if( xy == Y ) {
        HN.Callbacks.setHeight(index, new_val);
      } else {
        HN.Callbacks.setWidth(index, new_val);
      }
    };
