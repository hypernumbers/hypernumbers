...
HN.Callbacks.post = function(cell, value)
{
  var fullurl = document.location.pathname
    + HN.Util.to_b26(cell.x)+cell.y+"?attr";

  if(value == "") {
    $.post(fullurl, "<delete><value/></delete>",null,"xml");
  } else {
    $.post(fullurl,"{\"formula\":\""+value+"\"}",null,"json");
  }
};
...
HN.Layout.Panes.prototype.refresh = function()
{
  ...
  this.draw_index(Y, true, true, 0, 0, 1,
                  (this.rows * HN.Layout.Pane.DEF_ROWS) + 1);
  this.draw_index(X, true, true, 0, 0, 1,
                  (this.cols * HN.Layout.Pane.DEF_COLS) + 1);
...
HN.Layout.Selection.prototype.calculate_input_width = function()
{
  ...
  if((this.hidden_input.offsetWidth+15) > width) {
...
HN.Layout.Selection.prototype.rebuild_input = function()
{
  this.input.blur();
  this.formula.blur();

  this.cell_dom = this.layout.panes.get_cell(this.cell.y, this.cell.x);
  var val  = this.layout.s.cell(this.cell.y, this.cell.x).value || "";
...
HN.Layout.Selection.prototype.select = function(key, event)
{
  ...
  if(!event.shiftKey)
  {
    ...
    if(key == HN.Keys.DOWN && this.cell.y == b.y2 && b.y1 != b.y2) {
      ...
      b.y2++;
    } if(key == HN.Keys.UP && this.cell.y == b.y1) {
      b.y2--;
      ...
      b.x2++;
    } if(key == HN.Keys.LEFT && this.cell.x == b.x1) {
      b.x2--;
...
HN.Layout.Selection.prototype.is_range = function()
{
  return this.state == HN.States.EDIT_FULL_RANGE
    || this.state == HN.States.EDIT_LIGHT_RANGE;
//      || this.state == HN.States.SELECTED_RANGE;
};
