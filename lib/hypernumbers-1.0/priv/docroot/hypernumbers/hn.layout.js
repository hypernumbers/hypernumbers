...
{
  this.state   = HN.Layout.Selection.INIT;
  this.layout  = layout;
  ...
  this.coord       = {"x1":0, "y1": 0, "x2": 0, "y2": 0};
  this.bounds      = {"x1":0, "y1": 0, "x2": 0, "y2": 0};
...
HN.Layout.Selection.prototype.dragdown = function(e)
{
  ...
  this.state = HN.Layout.Selection.DRAGGING;
  ...
  var mousemove = function(e) {
    ...
    that.dragpos = {"x":x[0], "y":y[0]};
    that.show_drag_selection();
  };
  ...
  var mouseup = function(e) {
    ...
    that.state = HN.Layout.Selection.SELECTED;
  };
...
{
  // Clear old selection
  if(this.state == HN.Layout.Selection.SELECTED) {
    this.cell.style.zIndex = this.zindex;
    this.cell = null;
  }
  ...
  if(   this.state == HN.Layout.Selection.INIT
     || this.state == HN.Layout.Selection.SELECTED)
  {
    ...
    this.zindex = this.cell.style.zIndex;
    //this.cell.style.zIndex = 8500;

    this.state = HN.Layout.Selection.START_DRAGGING;
...
HN.Layout.Selection.prototype.show_drag_selection = function()
{
  if(this.bounds.x1 <= this.bounds.x2) {
    this.bounds_ord.x1 = this.bounds.x1;
    this.bounds_ord.x2 = this.bounds.x2;
  } else {
    this.bounds_ord.x1 = this.bounds.x2;
    this.bounds_ord.x2 = this.bounds.x1;
  }

  if(this.bounds.y1 <= this.bounds.y2) {
    this.bounds_ord.y1 = this.bounds.y1;
    this.bounds_ord.y2 = this.bounds.y2;
  } else {
    this.bounds_ord.y1 = this.bounds.y2;
    this.bounds_ord.y2 = this.bounds.y1;
  }
  ...
  var npos = pos(this.dragpos, this.bounds_ord);
...
HN.Layout.Selection.prototype.draw_drag = function(top, left, bottom, right)
{
  ...
  var cell = this.layout.panes.get_cell(top, left),
    toppx  = parseInt(cell.style.top)
             + parseInt(cell.parentNode.parentNode.style.top),
    leftpx = parseInt(cell.style.left)
             + parseInt(cell.parentNode.parentNode.style.left);
  ...
  this.dragged.style.top  = (toppx) + "px";
  this.dragged.style.left = (leftpx) + "px";
...
{
  var sheet = this.layout.s, wrapper = this.wrapper;

  if(this.bounds.x1 <= this.bounds.x2)
  {
    var left = this.coord.x1 - 1,
      startx = this.bounds.x1,
        endx = this.bounds.x2 + 1;
  }
  else
  {
    var left = this.coord.x2 - 1,
      startx = this.bounds.x2,
        endx = this.bounds.x1 + 1;
  }

  if(this.bounds.y1 <= this.bounds.y2)
  {
    var top = this.coord.y1 - 1,
     starty = this.bounds.y1,
       endy = this.bounds.y2 + 1;
  }
  else
  {
    var top = this.coord.y2 - 1,
     starty = this.bounds.y2,
       endy = this.bounds.y1 + 1;
  }
