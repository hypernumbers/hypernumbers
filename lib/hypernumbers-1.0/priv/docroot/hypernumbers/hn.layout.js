...
HN.Layout = function(s /*sheet*/) 
{
    ...
    function calcScrollbarPos(xy)
    {
        ...
        pos = track_s * pane_pos / s[xy.dimension];
    ...
    {
        var track_s = size[xy.str] - 32,
        bar_s = track_s * track_s / s[xy.dimension]; // track_s * %
     ...
     * Drag box pressed
     */
    function dragBoxPressed(c_event, xy)
    {
        var bar = scrollbar[xy.str],
        attr = (xy == Y) ? "clientY" : "clientX",
        origin = prev = c_event[attr],
        track_s = size[xy.str] - 32;
        
        function scroll(m_event) {
            var delta = m_event[attr] - origin;
            var est_dist = s[xy.dimension] * Math.min(1, Math.abs(delta) / track_s);
            var res = snapToDistance(delta > 0, est_dist, xy);
            if (res.dist != 0) {
                // snapped to a new cell
                doScroll(res.offset, res.dist, xy);  
                origin = m_event[attr];
            } else {
                // temporarily move slider
                var pos = parseInt(scrollbar[xy.str].style[xy.coord],10);
                doScrollbarPos(pos + (m_event[attr]-prev), xy);
            }
            prev = m_event[attr];
        };
    ...
    function scrollBy1(forward, xy)
    {
        ...
        if (forward)
            doScroll(1, -s[f](s.offset[xy.str]), xy);
        else
            doScroll(-1, s[f](s.offset[xy.str]-1), xy);
    };

    function doScroll(offset, dist, xy)
    {
        ...
        if ((calcScrollbarPos(xy) >= scrollmax[xy.str]) && forward) {
            s.extend(xy, 2);
