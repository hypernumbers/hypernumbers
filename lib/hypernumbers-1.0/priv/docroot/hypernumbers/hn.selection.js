...
HN.Layout.Cell.prototype.calculateWidth = function()
{
  ...
  do {
    tmpwidth += sheet.col_width(this.selection.cell.x + index);
...
HN.Layout.Cell.prototype.insert_range = function(range)
{
  ...
  var index = Math.max(f.lastIndexOf("="), f.lastIndexOf(","), f.lastIndexOf("("));
  var formula = f.substring(0, index+1) + HN.Util.range_to_str(range);
...
{
  this.c.end_edit();
  this.bounds_ord = this.order_bounds(this.bounds);
...
{
  this.layout.toolbar.dialog_open(HN.Util.id("newpage"));
};

HN.Layout.Selection.prototype.menuAction_import = function(e)
{
  this.layout.toolbar.dialog_open(HN.Util.id("import"));
};
...
{
  this.layout.toolbar.dialog_open(HN.Util.id("mark"));
};

HN.Layout.Selection.prototype.menuAction_tour = function(e)
{
  this.layout.toolbar.run_intro();
};
...
HN.Layout.Selection.prototype.menuAction_pastelink = function(e)
{
  ...
  for( var y = 0; y <= b.y2 - b.y1; y++ ) {
    ...
    for( var x = 0; x <= b.x2 - b.x1; x++) {
      arr[y][x] = "="+clipboard.path+HN.Util.coord_to_ref({x:x+b.x1, y:y+b.y1});
    }
  }
