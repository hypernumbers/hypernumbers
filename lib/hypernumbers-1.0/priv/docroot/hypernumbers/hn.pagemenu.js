...
HN.PageMenu.NavControl = function(input, tree, selectnode_fn) 
{
    ...
    public.startIfNeeded = function() {
        input.focus();
        if (timer === null)
            timer = setInterval(intervalPoll, POLL_INTERVAL);
    };
    ...
    public.setNode = function(n, p) {
        ...
        cur_node = n;
        input.val(p);
        input.selectRange(p.length, p.length);
        previous = p;
    };
    ...
    input.keydown(function(e) {
        if (e.keyCode >= LEFT_ARROW && e.keyCode <= DOWN_ARROW) {
            ...
            var full = input.val(), real = full;
            if (auto_start) {
                real = full.slice(0,auto_start);
                auto_start = null;
                input.val(real);
            }
            clearTimeout(timer);
    ...
    function completion(text, pre, node) {
        ...
        for (var i = 0; i < chi.length; i++) {
            ...
            if (res !== false) {
                auto_start = text.length;
                input.val(text + res + "/");
                input.selectRange(auto_start, input.val().length);
                return;
            }
        }
    };
