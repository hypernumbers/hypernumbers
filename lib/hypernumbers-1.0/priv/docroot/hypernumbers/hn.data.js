...
HN.Data.prototype.addPage = function(path) 
{
    ...
    this.pages[path] = new HN.PageData(path, this.options, fun);
};
...
HN.Data.prototype.updater = function(time)
{
    ...
    var fun = function(data) {
        if( typeof data.timeout == "undefined" ) {
            for( var i = 0; i < data.msgs.length; i++ ) {
                var page = that.pages[data.msgs[i].path];
    ...
    this.update = $.getJSON("/?updates="+time+"&path="+this.pathstr, fun);
};
...
HN.PageData.prototype.abort_update = function() {
    this.update.abort();
};

HN.PageData.prototype.load_pages = function()
{
    var that = this;
    
    $.ajax({
        url: this.path+"?pages", dataType: "json",
        success:   function(data) {
            that.pages = data;
...
HN.PageData.prototype.load_functions = function()
{
    var that = this,
