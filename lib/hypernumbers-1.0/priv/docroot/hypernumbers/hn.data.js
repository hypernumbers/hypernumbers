...
{
  var that = this;
  $.getJSON(that.url + "?updates="+time, function(data) {
...
HN.Data.prototype.handle_refresh = function(msg)
{
  ...
  $.ajax({
    ...
    success:   function(data) {
      that.data = data;
      HN.Util.dispatch(that.options.dataReloaded, []);
    }
  });
};
...
HN.Data.prototype.poke_cell_value = function(cell, value) {
  console.log(cell);
  if (this.data.cell[cell.y]) {
    if (this.data.cell[cell.y][cell.x]) {
    } else {
      this.data.cell[cell.y] = [];
      this.data.cell[cell.y][cell.x] = [];
    };
  } else {
    this.data.cell[cell.y][cell.x] = [];
  };
  console.log("old value: " + this.data.cell[cell.y][cell.x]['value']);
  this.data.cell[cell.y][cell.x]['value'] = value;
  console.log("new value: " + this.data.cell[cell.y][cell.x]['value']);
};