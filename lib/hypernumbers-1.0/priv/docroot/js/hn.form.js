...
var dosubmit = function()
{
    ...
    else if(action == "Edit")
    {
        for(i=0; i< post_vars.length; i++)
        {
            var val = post_vars[i].input.val();
            var str = "<create><formula><![CDATA["+val+"]]></formula></create>";
            var dest = post_vars[i].destination;
            if(dest != "undefined")
            {
                $.post(root+dest+"?attr",str,null,"xml");
            }
        }
    }
};
...
var edit_cell = function(cell)
{
  ...
  var onclick = function()
  {
    ...
    var input = $("<textarea class=\"edit_cell\"></textarea>").val(value);
    var submit = $("<input type='submit' value='save' />").click(save);
    var cancel_btn = $("<input type='submit' value='cancel' />").click(cancel);
    var edit_div = $("<div class=\"edit\"></div>").append(input)
      .append("<br />").append(submit).append(cancel_btn);
    parent.append(edit_div);
    input[0].focus();
  };
  cell.click(onclick);
};
...
var handle = function(data)
{
    ...
    {
        $(data[1]).append($("<h3>"+data[2]+"<h3>"));
    }
    else if(data[0] == "select")
    {
        var str = $("<div class='row'><label>"+data[2]
          +"</label><br /><select></select></div>");
    ...
    else if(data[0] == "livetable")
    {
        ...
        for(var z = x[1]; z <= y[1]; z++)
        {
            ...
            for(var n = x[0]; n <= y[0]; n++)
            {
                ...
                if(data[2] == "editable")
                {
                  var cell = td.find(".cell").addClass("editable")
...
$(function()
{
  ...
  {
    var range = $(data).find("ref[ref=A1] value").text();
    action =  $(data).find("ref[ref=B1] value").text();
    ...
    var editdata = function()
    {
      var input = function()
      {
        ...
        {
          el.val($(data).find("value").text());
        };

        $.get(root+this.destination+"?attr",change);
      };
      $(post_vars).each(input);
    };

    var callback = (action == "Edit") ? editdata : null;

    if(range != "")
      get_sorted_attr(datapath,range,handle,callback);

  };
  var datapath = "";
  var get_path = function(data)
  {
    datapath = $(data).text();
    $.get(datapath+"a1:b1?attr",load);
  };
  $.get(document.location.pathname+"?attr=formpath",get_path);
});
