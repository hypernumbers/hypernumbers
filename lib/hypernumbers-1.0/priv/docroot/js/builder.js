...
var build_edit_form = function(element,type)
{
  ...
  element.find(".rendered").empty().append(
    $(rendered[type].replace("{value}",val)));
...
function setup_form(path)
{
  ...
  var save = function()
  {
    ...
    $.post(root+"?attr","<delete><nochildren /></delete>",null,"xml");
    $("li.form-item").each(each);

    var range = "a3:e"+(data.length+2);
    $.post(root+"a1?attr","<create><formula>"+range
    ...
    var post_str = "<create><formpath>/forms"+path+"</formpath></create>";
    $.post(path+"?attr",post_str,null,"xml");
    $.post(path+"?attr","<create><gui>form</gui></create>",null,"xml");
  ...
  var load = function(data)
  {
    ...
    if(range != "")
    {
      $("#action").val($(data).find("ref[ref=B1] value").text());
      var end = function()
      {
  ...
  $.get(root+"a1:b1?attr",load);

  $("#add_item,#save").unbind(".builder");
  $("#add_item").bind("click.builder",add);
  $("#save").bind("click.builder",save);
  $(".form-item").each(init);
  $("#form").sortable({});
};