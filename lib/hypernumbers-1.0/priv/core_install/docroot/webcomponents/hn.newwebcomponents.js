...
HN.NewWebComponents.reload_tabs = function () {
    ...
    i, id, widthfn, tabwidth;
    tabwidth = $(".ui-tabs-panel").width();
    widthfn = function (idx, div) {
        var width = $(div).width(),
        leftmargin = (tabwidth - width)/2;
        if (leftmargin > 0) {
            $(div).css("margin-left", leftmargin + "px");
        }
    };
    $(".ui-tabs-panel > div").each(widthfn);
...
HN.NewWebComponents.reload = function () {
    ...
    // Tabs
    HN.NewWebComponents.reload_tabs();
	  ...
	  $("a img").parent().css("border-bottom", "none");    
        ...
        if (HN.isIE7) {
            ...
            $(".hn_inner > div > .hn_sld_menu")
                .parent().parent().parent().css("z-index", 250);
            $(".hn_inner > div > .hn_sld_menu")
