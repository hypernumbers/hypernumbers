...
HN.Phone = function () {
    ...
    setup_twilio_fn, setup_phone_fn,
    setup_email_fn, setup_panels_fn,
    ...
    format_groups_fn = function (groups) {
        ...
        for (g = 0; g < groups.length - 1; g = g + 1) {
            html = html + groups[g] + ", ";
        }
    ...
    dialpad_keydown_fn = function (e) {
        ...
        if (e.keyCode === 46 || e.keyCode === 8 ||
            ...
            //or a plus (SHIFT 187 or SHIFT 61 or SHIFT 107) stop it
    ...
    email_send_click_fn = function () {
        ...
        success_fn = function (Response) {
            ...
            tag = hour + ":" + min + ":" + sec;
            if (Response === "success") {
                $("#hn-email-response").html("Message sent to " +
                                                   email_to + " at " + tag);
            } else {
                $("#hn-email-response").html("Message sending failed at " + tag);
            }
            setup_email_fn();
        };
    ...
    setup_twilio_fn = function () {
        ...
        var setupFn, readyFn, errorFn, incomingFn, connectFn, disconnectFn; 
        
        Twilio.Device.setup(api.phonetoken);
        
        setupFn = function (device) {
            console.log("ready...");
        };

        Twilio.Device.ready(setupFn);
    ...
    setup_panels_fn = function () {
        
        var showEmail, showDialPad, panels;
        
        showEmail = (api.email_permissions === "fixed all" ||
    ...
    api.setup = function () {
        ...
        // now run setup for twilio if there is a phone
        if (api.twilio_capabilities.phone_out ||
          api.twilio_capabilities.phone_in) {
            setup_twilio_fn();
        }
