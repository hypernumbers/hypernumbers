...
HN.Phone = function () {
    ...
    // Now register the phone with the server
    ...
    register_phone = function () {
      console.log("registering phone");  
        ...
        register = function () {
            var args, path;
            args = JSON.stringify({"user": "test@hypernumbers.com"});
            path = "/_services/phone/register/";
            console.log("registering...");
            api.xhr = $.ajax({"data"     : args,
                              "type"     : "POST",
                              "url"      : path,
                              "dataType" : "json",
                              "success"  : register_phone});
        };
        
        setTimeout(register, 0);
    };

    // so just register it already
    register_phone();
