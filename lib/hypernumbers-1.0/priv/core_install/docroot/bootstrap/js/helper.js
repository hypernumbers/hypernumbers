...
HN.BootstrapHelper.fixPadding = function(type) {
    $(type).parent().css("padding", 0);
    if ($(type).parent().parent().hasClass("hn_inner")) {
        $(type).parent().parent().parent().css("padding", 0);
    };
};
...
HN.BootstrapHelper.reload = function() {
    ...
    // now sort out the padding for buttons
    HN.BootstrapHelper.fixPadding(".hn-btn-large");
    HN.BootstrapHelper.fixPadding(".hn-btn-small");
};