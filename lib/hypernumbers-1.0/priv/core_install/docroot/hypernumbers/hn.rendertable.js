	...
	var line_view = function(row) {
		...
		$("#lv_new").click( function(e) {
		  ...
		  // done) that calls line_view instead.
                  ...
                  table.bind("indexEnd", function(e) {
                    line_view(row);
		...
		$("#lv_first").click( function(e) {
		  e.stopPropagation();
		  line_view(display2row(1)); 
		});

		$("#lv_last").click( function(e) { // TODO not working?
		  e.stopPropagation();
		  line_view(display2row(displayCount())); 
		});

		$("#lv_prev").click( function(e) {
		  e.stopPropagation();
...
var open_dialog = function (id) {
    ...
    var enable_move_dialog = function(header) { 
      return function(e) {
        ...
        header.bind("mousemove", move_dialog);
        header.css("cursor", "move");
        $("body").bind("selectstart", disable_text_selection);
        e.preventDefault();
      };
    }(header);
    ...
    var disable_move_dialog = function(header) { 
      return function(e) {
        e.stopPropagation();
        header.unbind("mousemove", move_dialog);
        header.bind("mousedown", enable_move_dialog);
        header.css("cursor", "auto");
        $("body").unbind("selectstart", disable_text_selection);
        e.preventDefault();
      };
    }(header);

    header.bind("mousedown", enable_move_dialog);
    header.bind("mouseup",  disable_move_dialog);
    ...
    // TODO bound with min, max sizes, and auto-adjust scrollable content.

    var resize_dialog = function(dialog) {	
      return function(e) {
        e.stopPropagation();
    ...
    var enable_resize_dialog = function(resizer) {
      return function(e) {
        ...
        resizer.bind("mousemove", resize_dialog);
        $("body").bind("selectstart", disable_text_selection);
        e.preventDefault();	
      };
    }(resizer);
    ...
    resizer.bind("selectstart", function(e) { return false; });

    resizer.bind("mousedown", enable_resize_dialog);
    resizer.bind("mouseup",  disable_resize_dialog);
/*
    resizer.bind("mouseout", function(e) {
      resize_dialog(e);
      disable_resize_dialog(e); 
    });
*/
