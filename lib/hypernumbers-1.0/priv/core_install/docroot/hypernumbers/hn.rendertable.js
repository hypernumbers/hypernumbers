...
HN.TableRender = function () {
	...
	var line_view = function(row) {
		...
		if (first) {
		    ...
		    html += "<td><a id='lineviewbtn" + iy + "' class='lineviewbutton'><img src='/hypernumbers/right.png' /></a></td>";		    
		}
                html += "</tr>";
...
var open_dialog = function (id) {
    var dialog = $("#" + id),
        css = {"left": (($(window).width() - (dialog.width())) / 2)   + $("body").scrollLeft(),
               "top" : (($(window).height() - (dialog.height())) / 2) + $("body").scrollTop() };
