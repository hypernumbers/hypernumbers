...
var open_dialog = function (id) {
    ...
    // TODO if dialog.height() >= win_height then reduce height and
    // add scrollbars.

    // We use max(0, ...) to ensure that top is not off of the screen

    var css = {"left": Math.max(0, ((win_width  - (dialog.width()))  / 2) + $("body").scrollLeft()),
               "top" : Math.max(0, ((win_height - (dialog.height())) / 2) + $("body").scrollTop()) };
