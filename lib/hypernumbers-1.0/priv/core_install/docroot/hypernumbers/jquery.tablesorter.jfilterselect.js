...
// Known Issue: when used with zebra widget, zebra is not redrawn.
    ...
    if ($('#filter-column-'+table_id+'-0').length==0) { 
      $('th',table).each(function(n,e){
        ...
        if ($(e).attr('jFilterSelect')==1){
          ...
          f.append('<select id="filter-select-for-'+table_id+'-column-'+n+'" size="1"></select>');
  ...
  jFilterRun: function(table){
    ...
    $('tbody tr',$(table)).each(function(n,r){
      $('td',$(r)).each(function(i,f){
	    ...
	    var pass = ($(f).text() == myFilterColumn.val());
            $(f).attr('passFilter', pass);
        } 
        else {
            var myFilterText   = $('#filter-input-for-'+$(table).attr('id')+'-column-'+i);
            if (myFilterText.length > 0) {
		var pass = ($(f).text().toLowerCase().indexOf(myFilterText.val().toLowerCase()) != -1);
		$(f).attr('passFilter', pass);
            }
        }
      });
