Finder = function(element, json, opts)
{
    ...
    var bindFn, dblclickFn, itemsFn,
    rightclickopenFn, rightclickdeleteFn, rightclicknewFn, rightclicksaveasFn,
    rightclickuploadFn;
...
*/
    ...
    var init = function() {
        ...
        rightclickopenFn = function (obj) {
            var pathStr = nodeIdToPathStr($(obj).attr("id"));
            hn.ctrlPanel.pageSelected(HN.Util.pathToList(pathStr));
            api.select(HN.Util.pathToList(pathStr));
            api.openPageByPath(pathStr);
        };

        rightclickdeleteFn  = function (obj) {
            var pathStr = nodeIdToPathStr($(obj).attr("id"));
            // close the dialog box
            $("#ctrlclose").mousedown();
            HN.Util.delete_page(pathStr);
        };

        rightclicknewFn = function (obj) {
            var pathStr = nodeIdToPathStr($(obj).attr("id"));
            // close the dialog box
            $("#ctrlclose").mousedown();
            hn.pagesPanel.open_new(pathStr);
        };

        rightclicksaveasFn = function (obj) {
            var pathStr = nodeIdToPathStr($(obj).attr("id"));
            // close the dialog box
            $("#ctrlclose").mousedown();
            hn.pagesPanel.open("sitesaveastemplate");
        };

        rightclickuploadFn = function (obj) {
            var pathStr = nodeIdToPathStr($(obj).attr("id"));
            // close the dialog box
            $("#ctrlclose").mousedown();
            hn.pagesPanel.open("siteuploadxls");
        };

        // Dont show the save as template menu unless the user
        // is an admin
        if (!hn.is_admin) {
            itemsFn = function() {
                return {
                    "open" : {
                        "label"  : "Open Page",
                        "action" : rightclickopenFn
                    },
                    "new" : {
                        "label"  : "New Page",
                        "action" : rightclicknewFn
                    },
                    "delete" : {
                        "label"  : "Delete Page",
                        "action" : rightclickdeleteFn
                    } //,
                    // "upload .xls" : {
                    //    "label" : "Upload .xls File",
                    //    "action" : rightclickuploadFn
                    // }
                };
            };
        } else {
        ...
        bindFn = function(e,d) {
	      ...
	      $($wrapper).bind("select_node.jstree", bindFn);
        ...
        dblclickFn = function (e) {
	      ...
	      $($wrapper).delegate("li", "dblclick", dblclickFn);
