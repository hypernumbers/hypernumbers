Finder = function(element, json, opts)
{
    ...
    var memoizePath = function(pathStr) {
...
*/
    ...
    var init = function() {
	      ...
	      if (!node)        node = root;
 	      if (!currentPath) api.select(HN.Util.pathToList(node));
        
        var root_id = pathStrToNodeId(root),
	      ...
	      if (model) {
	          ...
	          for(var i=0; i<currentPath.length; i++) {
		            opens.push(pathStrToNodeId(HN.Util.listToPath(currentPath.slice(0,i+1))));
	          }
	      }
	      $($wrapper).jstree({
            "core"    : { 
