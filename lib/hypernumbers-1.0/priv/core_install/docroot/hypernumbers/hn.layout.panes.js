...
HN.Layout.Panes = function (layout, selection, sheet) {
    ...
    function drawIndex(xy, append, del, top, left, start, end) {
        ...
        var i, val,
            index = [],
            axiss = axis[xy.str],
            style = (xy === Y) ? "padding:3px 2px 0px 0px;border-bottom:1px " +
            "solid #aaa;" + "border-right:1px solid #aaa;text-align:right;" +
            "vertical-align:top;"
    ...
    function fixPreviewsAndIncs () {

        var previews, includes, i, newheight;

        previews = $(".hn_html_preview");
        includes = $(".hn-include");
        
        for (i = 0; i < previews.length; i++) {
            newheight = $(previews[i]).parent().height() - 4;
            $(previews[i]).height(newheight);
        }

        for (i = 0; i < includes.length; i++) {
            newheight = $(includes[i]).parent().height();
            $(includes[i]).css("height", newheight + "px");
        }
    };

    function bindWikiSelects () {
        var x, y;
        for (y = 0; y < rows; y += 1) {
            for (x = 0; x < cols; x += 1) {
                 HN.Util.setupInlineSelect("spreadsheet", p[y][x].uniqpaneid);
            }
        }
    }
    ...
    function shift_x(left) {
        ...
        for (y = 0; y < len; y += 1) {
            ...
            if (y === 0) {
                ...
                lleft = left ? parseInt(p[0][x - 1].div.style.left, 10) +
                    p[0][x - 1].width
