...
LangStr.prototype.get_langs = function () {
    var langs = new Array(),
        ...
        x;
    for (x in this.frags) {
        if (this.frags.hasOwnProperty(x)) {
            langs[i] = x;
            i = i + 1;
        }
    }
...
function load_language_strings(oLangStr) {
    ...
    oLangStr.add_frag("23", "fecha y tiempo", "yes");
}
...
Focus.prototype.changeFocus = function (val) {
    var newElem = document.getElementById(val),
        oldElem;
    if (this.focuspoint !== "") {
        oldElem = document.getElementById(this.focuspoint);
...
function writeLang(language) {
    var lang = language.replace("language_", ""),
        strings = oLangStrFrmBuilder.get_frags(lang),
        elem,
        x;
    
    for (x in strings) {
        if (strings.hasOwnProperty(x)) {
...
function setup_langs() {
    ...
    while (i < len) {
        html += "<img id=\"language_" + langs[i] + "\" src=\"/img/flags/" +
            langs[i] + ".png\">&nbsp;";
        i = i + 1;
    }
    // Now insert the flag fragments
    insertPoint = document.getElementById("formbuilder_lang");
    ...
    while (i < len) {
        (function (index) {
            id = "language_" + langs[i];
            HN.Util.addEvent(HN.Util.id(id), "click", function (e) {
                setFormBuilderLang(e, id);
            });
        })(i); 
        i = i + 1; 
    }
