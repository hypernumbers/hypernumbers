...
var open_dialog = function (id, adjust_content) {
    var dialog = $("#" + id),
    ...
    var move_dialog = function(dialog,header) {
      return function(e) {
        e.stopPropagation();
        var px = HN.Util.bounded(parseInt(dialog.css('left')) + e.pageX - x, 0, win_width - dialog.width()),
	    py = HN.Util.bounded(parseInt(dialog.css('top')) + e.pageY - y, $("body").scrollTop(), $("body").scrollTop() + win_height - header.height());
    ...
    if (adjust_content == undefined) {
        adjust_content = function(dialog, content, footer) {
            return function() {
                var top = content.offset().top - dialog.offset().top;
                content.height(dialog.height() - top - footer.height() - 12);
