...
HN.Layout.Selection = function (layout, sheet) {
    ...
    api.gridContextMenu = function (e) {
        ...
        var height = $(el).height(),
            top = ((e.clientY + height) > document.body.clientHeight) ?  e.clientY - height : e.clientY;
    ...
    api.pasteValues = function (copied) {
        console.log("in api.pasteValues");
        api.state = HN.States.SELECTED_RANGE;
        HN.Callbacks.pasteValues(sheet.path(), api.bounds, 
                                copied.host + copied.path, 
                                copied.bounds);
        api.editPasteBounds(api.bounds, copied.bounds);
    };
    ...
    api.menuAction_paste = function (e) {
        console.log("in api.menuAction_paste");
    ...
    api.menuAction_pastevalue = function (e) {
        ...
        api.pasteValues(JSON.parse(unescape(localStorage.copied)));
        return true;
    };
    ...
    api.menuAction_createform = function (e) {
        ...
        if (width > 0) {
            for (y = 0; y < height; y++) {
                ...
                HN.Callbacks.set_cell(path, obj, label);
            }
            ...
            HN.Callbacks.set_cell(path, {"x":bnd.x1,"y":bnd.y2}, button);
        }
        return true;
    };
...
HN.Layout.Editor = function (layout, selection, sheet) {
    ...
    function setSelectionToEnd() { 
        var last = getLastTextNode(editor);
        setRange(last, last.length, last, last.length);
    }
    ...
    api.quitEdit = function() {
        ...
        selection.cell_dom.innerHTML =
            HN.Util.previewMedia((c && c.value) || "");
    ...
    api.end_edit = function () {
        ...
        if (isFormula) {
            ...
            tmpDiv.innerHTML = HN.Util.previewMedia(val);
            val = tmpDiv.textContent;
        }

        if (selection.is_light_edit()) {
            
            selection.cell_dom.innerHTML = HN.Util.previewMedia(val);
            if (c) {
                c.value = val;
            }
    ...
    api.setFormula = function (value) { 
        selection.formula.innerHTML = HN.Util.previewMedia(value);
    };
