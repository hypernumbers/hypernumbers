...
HN.Layout.Selection = function (layout, sheet) {
    ...
    // api.startFormatPainter = function () {
    //    api.state = HN.States.PAINTER;
    // };

    // api.endFormatPainter = function () {
    //    $("#paintformat").removeClass("active");
    //    api.state = HN.States.SELECTED_CELL;
    // };
    ...
    api.mousedown = function (e) {
       ...
       // if (api.state === HN.States.PAINTER) { 
       //     //select = function () { };
       //     select_start = function () { 
       //         HN.Callbacks.pasteStyle(sheet.path(), bounds, api.bounds)
       //         api.bounds = bounds;
       //         api.endFormatPainter();
       //     };
            //select_end = function () { };
       // }
...
HN.Layout.Editor = function (selection, sheet) {
    ...
    function is_formula(val) {
        return value()[0] === "=";
    }
    
    function insert_range(range) {
        var f = value(),
    ...
    function read_formula_bar(range) {
        setTimeout(function () {
            set(selection.formula.innerHTML);
            calculateWidth();
        }, 0);
    }
    ...
    function execCommand(cmd, ui, value) { 
    ...
    return {
        ...
        "read_formula_bar" : read_formula_bar,
