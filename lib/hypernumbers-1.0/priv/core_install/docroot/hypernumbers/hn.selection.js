...
HN.Layout.Selection = function (layout, sheet)
{
    ...
    api.c     = new HN.Layout.Cell(api, sheet);

    api.currentCellValue = function () {
        return (api.is_editing() && api.c.value()) || 
    ...
    api.do_keydown = function (e)
    {
                  ...
                  e.keyCode === HN.Keys.BACKSPACE))
        {
            ...
            setTimeout(function () {
                api.formula.value = api.c.value();                
                api.c.calculateWidth();
            }, 0);
        }
    };
    ...
    api.dragdown = function (e)
    {
        ...
        auto_scroll, stop_auto_scroll, mousemove, mouseup;
        ...
        auto_scroll = function () {
            ...
            offy   = layout.gridMargin.y() + layout.gridPos.top();
            offx   = layout.gridMargin.x() + layout.gridPos.left();
            var y = api.getNewOffset(auto.mouse.y - offy, Y);
            var x = api.getNewOffset(auto.mouse.x - offx, X);
            api.show_drag_selection({"x": x.index, "y": y.index});
        };
    ...
    api.show_drag_selection = function(drag)
    {
        ...
        var npos = pos(drag, api.order_bounds(api.bounds));
    ...
    api.highlightRange = function (highlight, range) {
        ...
        for (y = range.y1, height = 0; y < range.y2+1; y++) {

            rowh = sheet.row_height(y); 
            
            for (x=range.x1, tmpw=0; x < range.x2+1; x++) {
    ...
    api.build_clipboard_str = function () {
        ...
        api.bounds = api.order_bounds(api.bounds);
        for (y = api.bounds.y1; y <= api.bounds.y2; y++ ) {
            for (x = api.bounds.x1; x <= api.bounds.x2; x++ ) {
    ...
    {
        HN.Callbacks.mergeCells(sheet.path(), api.order_bounds(api.bounds));
        return true;
    };
