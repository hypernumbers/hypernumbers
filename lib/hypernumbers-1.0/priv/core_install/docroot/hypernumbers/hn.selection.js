...
HN.Layout.Cell = function (selection, sheet) {
    ...
    api.calculateWidth = function () {
        ...
        hidden.innerHTML = editor.innerHTML;
        
        //if (input.style.whiteSpace === "normal") {
            // xy = Y;
            // cell = sheet.cell(selection.cell.y, selection.cell.x);
            // t  = ((cell && cell.merge && cell.mergeWidth  || 
        ...
        fn     = "col_width";
        input.style.height =
            (sheet.row_height(selection.cell.y) - 0) + "px";
        // }
        
        do {
            tmp += sheet[fn](selection.cell[xy.str] + index);
            input.style[xy.dimension] = (tmp - 0) + "px";
            index += 1;
        } while ((val + 10) > tmp);
    };
