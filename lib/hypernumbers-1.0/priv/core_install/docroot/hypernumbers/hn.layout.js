...
HN.Layout = function(data) {
    ...
    api.states = { 
        ...
        "MOST_VIEWBUILDER" : { "width" : 0.5 }
    };

    api.state = api.states.FULL_SPREADSHEET;
    ...
    api.switchTab = function(sheet) {
        ...
        var perms = sheet.key("permissions");

        $("#typepanel div").hide();
        
        if( perms.champion == "_g/core/webpage" ) {
            $("#webpagectrl").show();
            $("#webpagectrl a").attr("href", sheet.path());
        } else { 
        }
        //api.resumeSelection();
    };
    ...
    api.gridResize = function() {
        ...
        var bodyWidth = $("body").width(),
            gridWidth = (bodyWidth - HN.Util.x_pos(grid)) * api.state.width;
        ...
        $("#ctrlpanel").css("top", parseInt($("body").height() / 2)+"px");
    };
    ...
    scrollBarPressed["bar"] = function(clk_event, xy) {
        ...
        function scroll(mve_event) {
            ...
            var ngridpos = ((currentTab.sheet[xy.dimension]() - 
                ...
                * percentageScrolled;
            var delta = ngridpos + api.gridPos[xy.coord]();
    ...
    function doScroll(offset, dist, xy, canExpand) {
        ...
        var spos = api.calcScrollbarPos(xy);
