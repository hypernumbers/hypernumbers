...
HN.UI.CtrlPanel = function() {
    ...
    var currentTab = null;
    var ajaxUpload = null;
    var defaultTpl = "<div id=\"hnbody\">"
        +"<link rel=\"stylesheet\" type=\"text/css\" "
        +"href=\"/templates/blank/default.css\" />"
        +"<div id=\"content\" class=\"droppable\"></div></div>";
    ...
    function newForm() { 
        ...
        var path = makePath( $("#newformpath").val() );
        var name = "_u/form/"+$("#newformname").val()
            .toLowerCase().replace(/[^a-zA-Z 0-9]+/g,'');
        ...
        $.post(hn.currentPath(), JSON.stringify(params), 
               function(data) {
                   $("#newformpath, #newformname").val();
                   window.location.hash = "path="+path+",view="
                       +path+":"+name;
               }, "json");
    };
    ...
    function initEvents () {
        ...
        $("#ctrldelete").bind("mousedown", function() {
            var npath = hn.finder.asString();
            var cb = function() {
                if (npath == hn.currentPath()) {
                    npath = HN.Util.pathToList(npath);
                    if (npath.length > 0) {
                        npath.length--;
                    }
                    document.location.href = HN.Util.listToPath(npath);
                } else {
                    window.location.reload(true);
                }
            };
            HN.Util.postPath(npath, {"delete": "all"}, cb);
        });
...
HN.Site = function() {
    ...
    public.currentPath = function() {
        return public.hashUrl.getParam("path") || 
    ...
    function urlChanged() {
        ...
        var panel = public.hashUrl.getParam("panel");
        ...
        if( view == "create" && builder.state() != "CREATE" ) {
            ...
            $("#viewbuilderintro, #mainviewbuilder, #closeviewbtn")
                .hide();
