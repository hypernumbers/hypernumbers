...
HN.UI.CtrlPanel = function () {
    ...
    function newWebPage() {
        ...
        var done1 = function (data) {
            HN.Callbacks.setView(HN.Util.pathToList(path), "_g/core/webpage",
                                 {"everyone":true, "groups":[]}, done);
        };
        
        HN.Callbacks.setChampion(HN.Util.pathToList(path), 
                                 "_g/core/webpage", done1);
    };
    ...
    function uploadComplete(file, responce) {
        document.location.href = responce.location +
...
var makeTabs = function () {
    ...
    var tmp = $("<a href='#' id='hometab'><strong>create new</strong></a>")
        .bind("mousedown", function(e) {
            e.preventDefault();
            hn.ctrlPanel.open();
        });
    
    $("#tabs").empty().append(tmp);
...
var sheetDataLoader = function (sheetPath) {
    ...
    if (sheetPath === hn.currentPath(HN.url)) {
        ...
        // Permissions Stuff
        $("#viewtable").attr("href", path + "?view=table");
        $("#viewspreadsheet").attr("href", path + "?view=_g/core/spreadsheet");
        $("#viewwebpage").attr("href", path + "?view=_g/core/webpage");

        var perms = hn.data.key(path, "permissions");
        
        if (perms.champion) {
            $("#viewas input[value="+perms.champion+"]")
                .attr("checked", "checked");
        } else {
            $("#viewas input[type=radio]").removeAttr("checked");
        }

        perms.views["_g/core/spreadsheet"].everyone
            ? $("#publicspreadsheet").attr("checked", "checked")
            : $("#publicspreadsheet").removeAttr("checked");
        perms.views["_g/core/webpage"].everyone
            ? $("#publicwebpage").attr("checked", "checked")
            : $("#publicwebpage").removeAttr("checked");
        perms.views["table"].everyone
            ? $("#publictable").attr("checked", "checked")
            : $("#publictable").removeAttr("checked");        
