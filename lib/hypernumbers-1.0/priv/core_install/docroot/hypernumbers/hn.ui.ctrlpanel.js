...
HN.UI.CtrlPanel = function () {
    ...
    var addMapping = function () {
        ...
        if (!isValid(to, "sheet")) {
            ...
            if (isIn(datauploadmapping, json.to)) {
    ...
    var bindMapEvents = function () {
        ...
        $("#hn_map_save").bind("click", function() { saveMap(); });

        $("#hn_map_load").bind("click", function() { loadMap(); });
    };
    ...
    var loadMap = function () {
         var name = $("#hn_map_select :selected").val(),
            successFun = function(data) { displayMap(data); };
