...
HN.UI.CtrlPanel = function () {
    ...
    function addValidation() {
        var sheet      = $("#hn_validation_sheet").val(),
            ...
            len        = datauploadbody.length;
        datauploadbody[len] = json;
        displayMapBody();
    }
    ...
    function displayMapBody() {
        var body = $("#hn_map_body"),
            html = "";
        body.html("");
        for (a in datauploadbody) {
            html = body.html(); 
            body.html(html + HN.Util.objToHTML(datauploadbody[a], a));
        };
    }

    function deleteElement() {
        var selected = $(".hn_selected").attr("data-index");
        datauploadbody.splice(selected, 1);
        displayMapBody();
    }
