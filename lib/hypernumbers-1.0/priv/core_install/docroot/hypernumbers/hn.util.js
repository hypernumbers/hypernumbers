...
HN.Util.makeAdvPerms = function(permissions) {
    var html = "",
        view = {};
    html += "<h3>New Group</h3><p>Give your new group a name, set some views for it and apply it to this page.</p>";
    // create the add group item
    html += "<input id='newgroup' type='input' width='auto'/><table><tbody><tr>";
    ...
    html += "</tbody></table><input id='addnewgroupbutton' type='submit' "
        +"class='button' value='Create Group' /><p id=newgroupfeedback>&nbsp;</p><hr />";
    html += "<h3>Add Users To A Group</h3><p>add some users etc...</p>";
    return html;
};

HN.Util.makePermsTable = function(permissions) {
    var html      = "",
        groups    = [],
        noOfViews = 0,
        headerrow = "",
    ...
    groups = HN.Util.dedup(groups);
    headerrow = "<tr><td></td><td><small>default</small></td>";
    for (g in groups) {
        if (groups[g] !== "admin") {
            headerrow += "<td><small>"+groups[g]+"</small></td>";
        };
    };
    headerrow += "<td class='publicoption'><small>public</small></td>"
        +"<td></td></tr>";
    for (v in views) {
        ...
        if (groups.length === 1) {
            ...
            +"<a href='/?view="+v+"'>"+HN.Util.capitalise(v)
    ...
    setTimeout(function () {
                   ...
                   if (user !== "anonymous") {
                    ...
                    // fugly bodge - layout only exists on the spreadsheet
                        if (typeof layout === "object" && layout.gridHasFocus) {
                           layout.grabFocus();
                        };
