...
HN.Util.makeAllowedViews = function () {
    ...
    v,
    views = hn.data.readViews(document.location.pathname.toLowerCase()),
    hasSpreadsheet = false,
    hasWebpage = false,
    hasWikipage = false,
    permissions =  hn.data.readPermissions(document.location.pathname.toLowerCase()),
...
HN.Util.makeSelect = function (list, selection, ref, uniqid) {
    var opts = "", start;
    for (x in list) {
    ...
    return "<select class='hn_inlineselect hn_inlineselect_" + uniqid +
        "' data-ref='" + ref + "'>" +
        opts + "</select>";
};
...
HN.Util.shorten = function(name) {
    var bits = name.split("@");
    switch (bits.length) {
    case 1: return "<p>" + name + "</p>";
    case 2 : return "<p title='" + name + "'><i>" + bits[0] + "&nbsp;(u)</i></p>";
    }
};
