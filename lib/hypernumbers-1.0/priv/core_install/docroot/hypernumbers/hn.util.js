...
/*global HN: false, hn: false, $: false, Y: false, X: false, localStorage: false, escape: false, unescape: false, window: false, jscolor: false, layout: false, confirm: false */
/**
 * @class HN.Util
 *
 */
...
HN.Util.isInWordpress = function () {
...
HN.Util.flash_msg = function (string) {
    $("#hn_save_msg").text(string);
    $("#hn_save_msg").fadeIn(100);
    $("#hn_save_msg").fadeOut(200);
    $("#hn_save_msg").fadeIn(100);
    $("#hn_save_msg").text("all changes saved");
};
...
HN.Util.setPassword = function (password) {
    ...
    HN.Callbacks.setPassword(password, setFn);
};
...
HN.Util.makeAllowedViews = function () {
    var html  = "",
    sep   = "<hr class='horiz_sep' color='#dddddd' background-color='#dddddd' />",
    v,
    ...
    // now change the vixo tab if the page is public
...
HN.Util.makeSelects = function (list, type, id, selected) {
    var selects = [],
        html      = "",
        i;
    ...
    for (i in list) {
        if (list.hasOwnProperty(i)) {
            if (list[i] === selected) {
                html += "<option selected='selected'>" + list[i] +
                    "</options>";
            } else {
...
HN.Util.makePermsTable = function (permissions, path) {
    ...
    gAndVs = HN.Util.remove_apis(gAndVs),
    ...
    headerrow += "<td class='publicoption'>public</td><td></td></tr>";
    ...
    for (v in gAndVs.views) {
        ...
        for (g in gAndVs.groups) {
            if (gAndVs.groups.hasOwnProperty(g)) {
                if (gAndVs.groups[g] !== "admin") {
                    rows += "<td><input data-perm-view='" + v + "' " +
                        " data-perm-group='" + gAndVs.groups[g] +
...
HN.Util.makeWikiSelect = function (input, selection, ref, uniqid) {
    var opts = "",
    start, x, list, arg, selected = false;
    if (input.select) {
        list = input.select;
    ...
    for (x in list) {
        if (list.hasOwnProperty(x)) {
            arg = list[x].toString();
            if (HN.Util.softEquals(arg, selection)) {
                selected = true;
    ...
    if (selected === false) {
        opts = "<option selected>" + selection + "</option>" + opts;
    }
...
// 01234 equals 1234
// 1.230 equals 1.23
// 123.0e+1 equals 1.23e+3
// True equals true
// fAlse equals false
HN.Util.softEquals = function (arg, selection) {
    if (arg === selection) {
        return true;
    } else if ((arg.toLowerCase() === true) || (arg.toLowerCase() === false)) {
            if (arg.toLowerCase() === selection.toLowerCase()) {
                return true;
            } else {
                return false;
            }
    } else if (parseFloat(arg) === parseFloat(selection)) {
        return true;
    } else if (parseInt(arg, 10) === parseInt(selection, 10)) {
        // mebbies an int match is not real
        // 'cos parseInt(...) truncates floats
        if ((parseInt(arg, 10) - parseFloat(arg) !== 0) ||
            (parseInt(selection, 10) - parseFloat(selection) !== 0)) {
            return false;
        } else {
            return true;
        }
...
HN.Util.getPreview = function (cell) {
    if (cell.preview) {
        // need to support the old and new previews
        if (cell.preview.txt) {
            return "<div class='hn_html_preview'>" +
...
HN.Util.initLogin = function (type) {
    var cookie = HN.Util.readCookie("auth"),
        ...
        doLogin;
    $("#forgotten_pwd").html("<a href=\"http://vixo.com/_forgotten_password/?site=" +
...
HN.Util.remove_apis = function(gAndVs) {
    var i, groups, views, filterGroupsFn, filterViewsFn;
    filterGroupsFn = function(index) {
    ...
    groups = gAndVs.groups.filter(filterGroupsFn);
    views = gAndVs.views;
    for (i in views) {
        if (views.hasOwnProperty(i)) {
            views[i] = views[i].filter(filterGroupsFn);
        }
    };
    return {groups: groups, views: views};
}
    ...
    var objPattern, arrData, arrMatches, strMatchedDelimiter, strMatchedValue,
	  ...
	  arrData = [[]];
		...
		while (arrMatches = objPattern.exec(strData)) {
			  ...
			  arrData[arrData.length - 1].push(HN.Util.trim(strMatchedValue));
		}
...
HN.Util.getPosition = function (arrayName, arrayItem) {
    var i, bleh;
...
HN.Util.delete_page = function (pathStr) {
    ...
    conf = confirm("Are you sure that you want " + "to delete " + pathStr + "?");
    json = {"delete": "all"};

    if (conf) {
		    HN.Util.postPath(pathStr, json, cb);
	  }
};
