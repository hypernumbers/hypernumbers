...
HN.Util.objToHTML = function(obj, index) {
    var html = "<div data-index='"+index+"'>",
    length = obj.length;
    for (x in obj) {
            if (x !== length) {
                html += x +
...
HN.Util.makeTemplates = function (id) {
    var templates = [],
        html      = "",
        t;
    templates = hn.templates;
    html = "<select id='" + id + "' name='templates'>";
    for (t in templates) {
