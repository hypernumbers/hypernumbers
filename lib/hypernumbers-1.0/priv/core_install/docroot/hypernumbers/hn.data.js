            ...
            if (typeof data.timeout === "undefined") {
                ...
                for (i = 0, len = data.msgs.length; i < len; i = i + 1) {
                    ...
                    if (msg.type === "style") {
                        ...
                        // render the page (if appropriate)
                        // ie if you are on a web/wiki page
                        if (options.render) {
                            options.render();
                        };
                    }
...
HN.PageData = function (url, options, loaded, tpl, parent)
{
    ...
    api.handle_delete_attrs = function(msg) {
        ...
        for (a in msg.attrs) {
            if (msg.attrs.hasOwnProperty(a)) {
                if (msg.reftype === "cell") {
                    ind = HN.Util.parse_ref(msg.ref);
                    delete api.data.cell[ind.y][ind.x][a];
                    HN.Util.dispatch(api.options.cellChanged, [api.path, ind.y, ind.x]);
               } else if (msg.reftype === "row") {
                   console.log("row is being cleared...");
                   delete api.data.row[ref];
               } else if (msg.reftype === "column") {
                   console.log("column is being cleared...");
                   col = HN.Util.from_b26(ref);
                   delete api.data.column[col];
               }
            }
        }
    };
    ...
    api.pokeCellValue = function (cell, value) {
        ...
        api.data.cell[cell.y][cell.x].__rawvalue = value;
    };
