HN.Data = function(options) {
    ...
    public.writeKey = function(path, key, data) {
        if( !pages[path] ) { 
            pages[path] = {};
        }
        if( !pages[path].data ) { 
    ...
    public.deleteKey = function(path, key) {
    ...
    public.addPage = function(path) {    
        ...
        pages[path] = new HN.PageData(path, options, fun, tpl, public);
    };

    public.removePage = function(path) {
        delete pages[path];
    };
...
HN.PageData = function(url, options, loaded, tpl, parent)
{
    ...
    public.load_data = function(loaded)  {
        ...
        args = ( public.path === document.location.pathname )
            ? {"view" : public.tpl}
        ...
        $.ajax({
            ...
            "error"    : function(data) {
                ...
                if( data.status == 401 ) {
                    HN.Util.dispatch(public.options.authError, [path]);
                }
            }
        });
    };
    ...
    public.delete_cell = function(msg) {
        ...
        if (msg.name === "merge") {
            HN.Util.dispatch(public.options.dataReloaded, [public.data]);
        }
    };
    ...
    public.pokeCellValue = function(cell, value) {
        public.ensure({
            "reftype" : "cell", 
