...
HN.Data = function (options) {
    ...
    var api           = {},
        rootPath      = document.location.pathname.toLowerCase(),
        opaque        = {},
        styles        = null,
        pages         = {},
        updateRequest = null,
    ...
    function updater(time) {
        ...
        var siteSuccessFun, dataSuccessFun, completeFun, f, paths, args;
        ...
        completeFun = function(data, status) {
            if (!clientAbort && status !== "success") {
                HN.Util.showDeadBox("Connection with the server has been " +
                                    "lost. Please refresh the page in " +
                                    "your browser");
            }
        };
...
HN.PageData = function (url, options, loaded, tpl, parent)
{
    ...
    api.load_data = function (loaded)  {
        ...
        var path = api.path,
            args = (api.path === document.location.pathname.toLowerCase()) ?
              {"view" : api.tpl} :
        
              {"via"  : document.location.pathname.toLowerCase(), 
               "view": api.tpl};
