...
HN.ToolBar = function (layout) {
    ...
    api.loadViews = function () {
        ...
        groupinputchangefn = function () {
            var view = $(this).attr("data-perm-view"),
            group = $(this).attr("data-perm-group"),
    ...
    api.wizardSelectFunction = function(f) {
        ...
        if (f.link) {
            link = f.fn.toUpperCase() + "<br /><a class='hn_documentation' " +
                "href=" + f.link + "?from=" + document.location +
                " target='_blank'>" +
                "(help and examples)</a>";
    ...
    api.displayFunctionsByCategory = function(catids) {
        ...
        for (i = 0; i < fs.length; i = i + 1) {
            name = fs[i].fn.toUpperCase();
            s = fs[i].inexcel ? name : name + " <- new!";
	          $("#wizfunlist").append(new Option(s, fs[i].index));
	          }
    };
    ...
    api.loadFunctions = function () {
        ...
        catids[wizcats[DefCat]] = [];
	      ...
	      $.get("/hypernumbers/fns_en-GB.json", function(d) {
            var fs = JSON.parse(d),
                  changeFun, clickFun, i, f, c, list = [], rlist = [],
                  index, rindex;
                  if (fs) {
	                    for (i = 0; i < fs.length; i = i + 1) {
		                      f = fs[i];
                      ...
                      // need to build an alphabetical list
                      // which keys back to the non-alphabetical one
                      rlist = list.slice(0);
                      rlist.sort();
                      ...
                      for (i = 0; i < list.length; i = i + 1) {
                          // the selections are numbered in the unsorted order
                          rindex = HN.Util.getPosition(list, rlist[i]) + 1;
			                    $("#wizcatlist").append(new Option(rlist[i], rindex));
                      }
    ...
    function enableEditMenu() {
        $("#editmenulist a").bind("mousedown", function (e) {
          e.preventDefault();
          layout.tabUI().menuItemPicked(e.target);
        });
    }

    function enableInsertMenu () {
        $("#insertmenulist a").bind("mousedown", function (e) {
          e.preventDefault();
          layout.tabUI().menuItemPicked(e.target);
        });
    }
    ...
    function deletepage () {
        HN.Util.delete_page(layout.currentSheet().path());
    };
    ...
    function enableContextMenu() {
        ...
        inFn = function (e) {
            ...
            $(".hn_gridcontextsubmenu").css("display", "none");
            $(".hn_gridcontextmenu_expand").css("background",
                     "url(/hypernumbers/right.png) no-repeat right 2px");
            $(this).css("background",
                        "url(/hypernumbers/down.png) no-repeat right 2px");
            top = $(e.currentTarget).offset().top;
            boxtop = $("#gridcontextmenu").offset().top;
            $(this).children("ul").css("top", top - boxtop - 2);
        ...
        $(".hn_gridcontextmenu_expand").hover(inFn, outFn);
    }

    function enableSiteMenu() {

        // you need to be an admin to mess with templates
        if (!hn.is_admin) {
            $("a[name=sitesaveastemplate]").hide();
        }

        var mousedown = function (e) {
            var target = $(e.currentTarget).attr("name");
            e.preventDefault();
            layout.tabUI().editor.quitEdit();
            if ((target === "sitemypages") ||
                (target === "sitesettings") ||
                (target === "sitedataupload") ||
                (target === "sitemanageapi") ||
                (target === "sitetour")) {
                hn.ctrlPanel.selectTab($(e.currentTarget).attr("name"));
                hn.ctrlPanel.open();
            } else if ((target === "sitesaveastemplate") ||
                       (target === "siteuploadxls")) {
                hn.pagesPanel.open(target);
            } else if (target === "sitenewpage") {
                hn.pagesPanel.open_new(hn.currentPath());
            } else if (target === "sitedelete") {
                deletepage();
            }
        };

        $("#sitemenulist a").bind("mousedown", mousedown);
    }
    ...
    function initViews() {
        ...
        $("#addcolleaguebutton").bind("click", function () {
            ...
            $("#invitecolleague").val("");
            HN.Callbacks.inviteUser(path, email, perms.champion, msg);
        });
    ...
    function dropMenus() {
        var i, el = $(".expand"),
            ...
            click = function (e) {
                ...
                // want to drop out of editing when using menus
                layout.tabUI().editor.quitEdit();
                ...
                if (name === "editmenulist") {
                    if (layout.tabUI().state === 3) {
                        $(".editviewlogs").show();
                    } else {
                        $(".editviewlogs").hide();
                    }
    ...
    function initEvents() {
        ...
        $("#wikiincrementform").bind("submit", function (e) {

            var path, range, value;
            e.preventDefault();
            value = $("#incrementsize").val();
            path  = layout.currentSheet().path();
            range = layout.tabUI().currentSelectedBounds();
            HN.UI.close_dialog(layout, "wikiincrementdialog");
            HN.Callbacks.set_input(path, range, {"inputincrement": value});
        });
        ...
        bottom   = $("#wizstatus"),
	      ...
	      funlist  = $("#wizfunlist"),
        wizbtnfn, formfn, formbtnfn, mergecellsfn, inlinestatusfn,
        inlinefn, inlinerichtxtfn,
        dropdownfn, inlinecheckboxfn, userdeffnsfn,
        userdeflayoutfn, ghostfn;
        ...
        formfn = function (e) {
            ...
            formula = "=form." + $(this).attr("data-input") + "(" + params + ")";
        ...
        formbtnfn = function (e) {
            var button = "=form.button(\"Submit Form\", " +
        ...
        inlinestatusfn = function(newstatus) {
        ...
        inlinefn = function (e) {
            inlinestatusfn("inline");
        };

        $("#inlineinput").bind("mousedown", inlinefn);

        inlinerichtxtfn = function (e) {
        ...
        $("#inlinerichtextinput").bind("mousedown", inlinerichtxtfn);

        inlinecheckboxfn = function (e) {
            inlinestatusfn("inlinecheckbox");
        };

        $("#inlinecheckbox").bind("mousedown", inlinecheckboxfn);

        userdeffnsfn = function () {
            HN.UserDefFns.configure();
            HN.Dialog.open_dialog(layout, "hn_makeuserdeffns");
        };
        ...
        ghostfn = function () {
            ...
            if (layout.currentSheet().cell(cell.y, cell.x).ghost) {
                status = false;
            } else {
                status = true;
            }
            HN.Callbacks.set_ghost(path, range, status);
        };

        $("#ghost").bind("click", ghostfn);
    }
    ...
    dialogButton("inlineincrementor", "wikiincrementdialog");
    initViews();
    initWikiSelect();
