...
*/
	...
	var l = $( document.createElement('label') ).html(a.name).attr("title", a.desc).attr("for", n);
	...
	switch (a.type) {
	    ...
	    if (a.variable) { a.variable.val(nc[1]); }
	      ...
	      var more = $( document.createElement('span') ).attr("id", "m" + n).attr("class", "wizrep").attr("title", "more").html("+")
	...
	if (x == null) {
	    alert("Invalid formulae.");
		 ...
		 (typeof(x.expression.fn) != "undefined")) {

	    var n = x.expression.fn.name;
	    if (typeof(n) == "undefined") {
		n = x.expression.fn; // Due to bug in expression parser
	    }
