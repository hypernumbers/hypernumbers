	...
	switch (a.type) {
 	...
 	case "infinite":
	    ...
	    var more = $( document.createElement('img') ).attr("id", "m" + n).
                attr("class", "wizrep").attr("alt", "Add").attr("src", "img/add.png")
		.click( function(a,f,d,z) {
			    return function(ev) {
					...
					function(a,f,d,z) {
					    return function(ev) {
						d.remove();
						var fl = $('label[id^="l' + nc[0] + '"]:first').addClass("req");

						// Update the tooltip to say that argument is required
...
*/
		content.width(dialog.width() - content.position().left - 4);
		bottom.width(content.width());

		var btn = content.find(".wizrep:first"),
		    ofs = 6 + (btn.length > 0 ? btn.width() : 0) + $.scrollbarWidth();

		bottom.find("#wiztext").width(bottom.width() - 4 - footer.find("a.resize:first").width());
		...
		funlist.attr("size", Math.floor((dialog.height() - funlist.position().top) / parseInt(funlist.css("line-height"))));
