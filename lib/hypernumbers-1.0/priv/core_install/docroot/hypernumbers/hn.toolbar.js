...
HN.ToolBar = function (layout) {
    ...
    api.loadViews = function(permissions, groups) {

        var table = HN.Util.makePermsTable(permissions),
            path = HN.Util.pathToList(layout.currentSheet().path()),
            perms = hn.data.key(layout.currentSheet().path(), "permissions"),
            clickPublic;
        // Fire the bitse in
        $("#permstable").html(table);

        // Yer basic permissions box is first up
        //
        // Now set up the functions and stuff
        // First up the default options
        $("#newviewas input[name=newdefaultpage]").bind("change", function() {

            perms.champion = $(this).val();
            hn.data.writeKey(path, "permissions", perms);            
      ...
      clickPublic = function() {
          ...
          perms.views[view].everyone = $(this).is(":checked");
          hn.data.writeKey(path, "permissions", perms);
          
          HN.Callbacks.setView(path, view, perms.views[view],  null);
      };
        ...
        $(".groupinput").bind("change", function() {
                                  ...
                                  if (checked) {
                                      ...
                                      pv.groups = HN.Util.remove(pv.groups, 
                                                                 a[1]);
                                  };
                                  hn.data.writeKey(path, "permissions", perms);
                                  HN.Callbacks.setView(path, a[0], 
                                                       perms.views[a[0]], null);
                                  });
        // Now (if necessary) make the advanced options panel
        //
        if (hn.data.is_admin) {
            var advperms = HN.Util.makeAdvPerms(permissions),
                path     = hn.currentPath(HN.url),
                perms    = hn.data.key(path, "permissions");
            $("#advperms").html(advperms);
            $("#newgroup").bind("change", function() {
                var divarray = [],
                    views    = [],
                    name;
                ...
                $("#newgroupfeedback").html("group added");
            });
            $("#advgroups").bind("click", function() {
                if ($("#permstable").css("display") === "block") {
                    $("#permstable").hide();
                    $("#newgroup").val("");
                    ...
                    layout.grabFocus();
                    $("#newgroup").focus();
                    $("#advgroups").html("basic options");
                } else {
                    $("#advperms").hide();
                    $("#permstable").show();
                    layout.resumeSelection();
                    $("#advgroups").html("advanced options");
               };
            });
            //$("#advgroups").show(); hide for the release
        } else {
            $("#advgroups").hide();                
        };
    };
    ...
    api.generateFunCategories = function () {
        ...
        var html    = "",
            ...
            i, len, fun, str, x, category, functionName;
        
        cat["All Functions"] = [];
        
        for (i = 0, len = functions.length; i < len; i += 1) {
    ...
    function dropMenus() {
        ...
        var i, el = $(".expand"),
            ...
            click = function (e) {
                ...
                var hide = function (e) {
                    // yeah its a bug but it will go when viewas goes...
                    if (e.target === menu ||
                        name === "viewas" || name === "newviewas" &&
    ...
    function initEvents() {
        ...
        $("#hometab").bind("mousedown", function(e) {
            e.preventDefault();
            hn.ctrlPanel.open();
        });
        
        // Old permissions stuff
        $("#viewas input[name=defaultpage]").bind("change", function() {
            var path = layout.currentSheet().path(),
                perms = hn.data.key(path, "permissions");
        ...
        $("#publictable, #publicspreadsheet, #publicwebpage")
            .bind("change", function() {
                var path  = layout.currentSheet().path(),
                    view  = $(this).attr("data-value"),
                    perms = hn.data.key(path, "permissions");
                
                perms.views[view].everyone = $(this).is(":checked");
                hn.data.writeKey(path, "permissions", perms);
                
                HN.Callbacks.setView(HN.Util.pathToList(path),
                                     view, perms.views[view],  null);
            });
        ...
        $("#forminput, #formtextarea, #formselect, #formradio")
            .bind("mousedown", function (e) {
                ...
                var formula = "=" + $(this).attr("data-input") +
                    "(" + params + ")";
