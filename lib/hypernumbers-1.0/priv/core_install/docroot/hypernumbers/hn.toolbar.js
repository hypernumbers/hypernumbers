...
HN.ToolBar = function (layout) {
    ...
    function imageButton() { 
        ...
        $("#insertimagedialog .cancel").bind("mousedown", function() { 
            HN.UI.close_dialog(layout, "insertimagedialog");
        });

        $("#insertimage").bind("mousedown", function () { 
            HN.UI.open_dialog(layout, "insertimagedialog");
        });

        $("#imageform").unbind().bind("submit", function(e) {
            e.preventDefault();
            ...
            if (layout.tabUI().is_editing()) { 
                ...
                HN.Callbacks.set_cell(path, cell, img);                
            } else {
                // TODO : restore cursor selection
    ...
    function comboColorPickers() {
        ...
        add_events("combocolorsbg","background-color",
                   function(obj) { return "#"+obj.getAttribute("data-bgcolor"); });
        add_events("combocolorsbg","color",
                   function(obj) { return "#"+obj.getAttribute("data-fontcolor"); });
        // the combo color fires 2 posts one for background, one for font
        add_events("combocolorstxt","background-color",
                   function(obj) { return "#"+obj.getAttribute("data-bgcolor"); });
        add_events("combocolorstxt","color",
               function(obj) { return "#"+obj.getAttribute("data-fontcolor"); });
    };
        ...
        click = function (e) {
            ...
            hide = function (e) { 
                ...
                if (HN.Util.is_inside(e.target, menu[0]) ||
                    HN.Util.is_inside(e.target, domParent)) { 
                    return;
                }
