...
*/
    ...
    api.getFunctionText = function(f) {
      ...
      for(var i=0; i<cnt; i++) {
	  args.push($("#wizarg"+i).val());
      }
	return f.fn + "(" + args.join(",") + ")";
    };
	...
	// Function to Resize the Function Wizard's Argument List
	    ...
	    adjust_arglist = function(dialog, content, bottom, footer) {
            return function() {
		var top = Math.round(content.offset().top - dialog.offset().top),
                    bot = bottom.height() + footer.height() + 36; // TODO
		...
		open_dialog("funwiz", adjust_arglist);
	    }
        });
