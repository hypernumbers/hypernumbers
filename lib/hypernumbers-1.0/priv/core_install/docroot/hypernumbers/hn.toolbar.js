...
HN.ToolBar = function (layout) {
        ...
        for(var i=0; i<f.args.length; i++) {
	          if (f.args[i].type == "variable") {
                ...
                // ignoring blanks at the end
	              var vs = api.getArgValues(f.args[i].attr, req);
                ...
                if (isValid && !vs.isValid) {
	              ...
	              var vs = api.getArgValues(f.args[i], req);
	              if (vs.args.length) {
	                  args.push(vs.args);
                    if (isValid && !vs.isValid) {
                        isValid = false;
	      ...
	      var g = function(x) {
	          if (x==null) {
		            ...
		            if (v !== null) {
		                ...
		                alert("Invalid argument. twazzock");
    ...
    api.createArgument = function(a,f) {
        var n = api.getClass(a);
	      ...
	      var l = $( document.createElement('label') ).text(a.name).attr("for", n).attr("class", "l" + n);

	      var z = $( document.createElement('input') ).attr("class", n);
        ...
        z.bind("change", function(f) {
	      ...
	      var e  = $( document.createElement('div') ).attr("id", "e" + n).
            append(l).append(z),
        es = [ e ];
	      ...
	      switch (a.type) {
 	        ...
 	        case "infinite":
	          ...
	          var more = $( document.createElement('img') ).attr("id", "m" + n).
                attr("class", "wizrep").attr("alt", "Add").attr("src",
                                                                "img/add.png")
		            .click( function(a,f,d,z) {
			              return function(ev) {
			                  ...
			                  more.unbind("click").empty().attr("alt", "Delete").
    ...
    api.createResizer = function(name,f,def) {
	      ...
	      var l = $( document.createElement('label') ).text(name).attr("for", n).
	      ...
	      var z = $( document.createElement('input') ).attr("id", n).attr("size", 5).
        ...
        z.bind("change", function(ev) {
		        ...
		        if (v.match(/\D/) || (parseInt(v) < 1)) {
	              alert("Please enter a number >= 1. dumbo");
	      ...
	      var zd = $( document.createElement('img') ).attr("class","wizrep").attr("alt", "Decrease").
            attr("src","/img/delete_bw.png").click(function (ev) { z.trigger("Decrease"); });

	      var zi = $( document.createElement('img') ).attr("class","wizrep").attr("alt", "Increase").
            attr("src","/img/add_bw.png").click(function (ev) { z.trigger("Increase"); });

	      var e = $( document.createElement('div') ).append(l).append(zd).append(z).
            append(zi);
    ...
    api.wizardAddArguments = function(f) {
	      ...
	      if (f.resize) {
	          var r = $( document.createElement('div') ).attr("class", "resizer");
    ...
    api.wizardSelectFunction = function(f) {
        ...
        if (f.link) {
	      ...
	      if (f.inexcel) {
	          desc += " <span class='hn_wiz_emphasis'>(Identical to Excel)</span>";
	      }
	      if (f.experimental) {
	      ...
	      var g = function(x) {
            if (x===null) return; // TODO?
	          ...
	          if (v !== null) {
		            var path  = layout.currentSheet().path(),
  		          cell  = {
		                "y": parseInt($("#celly").text()),
		                "x": parseInt($("#cellx").text())
		            },
		                cmd   = { "set" : { "formula" : "=" + v } };
                ...
                $("#funwiz").trigger("CloseDialog");
		            ...
		            alert("Invalid or missing argument. bampot");
		            api.setWizText(x.expression.error);
	          }
	      };
    ...
    function initWikiSelect() {
        ...
        var onfocusfun = function() {
            ...
            $("#hn_insertwikiselect").val(options.toString());
        };
        $("#hn_makewikiselect").bind("focus", onfocusfun);
        var onsubmit = function () {
            var selection = $("#hn_insertwikiselect").val().split(","),
            status, path, cell, range;
            if (selection === []) {
                status = "none";
            } else  {
                status = {"select": HN.Util.trimarray(selection)};
            };
            path  = layout.currentSheet().path();
            cell  = layout.tabUI().currentSelectedCell();
    ...
    function initEvents() {
	      ...
	      api.adjust_arglist = function(dialog, content, bottom, footer, funlist) {
            return function() {
                ...
                content.width(dialog.width() - 154);
		            ...
		            var btn = content.find(".wizrep:first"),
		            ...
		            content.find('label[id^="lwa"]').each(
		                function (width, ofs) {
			                  return function(idx,el) {
			                      var l = $(el),
			                      i = $("#" + l.attr("for"));
 			                      // Note: leave room for "+" and "-" buttons and scrollbar
			                      i.width(width - l.width() - ofs);
			                  };
		                }(content.width(), ofs)
                );

		            var lsz = parseInt(funlist.css("line-height")),
        ...
        $("#wizbtn").bind("mousedown", function (e) {
            if (funwiz.is(":visible")) {
                ...
                HN.Callbacks.setMark("opening function wizard box");
		            ...
		            $("#cellx").text(cell.x);
		            $("#celly").text(cell.y);
		            $("#cellid").text(HN.Util.refToStr({"type": "cell", "obj": cell}));
        ...
        $("#mergecells").bind("mousedown", function (e) {
            var bounds = layout.tabUI().order_bounds(layout.tabUI().currentSelectedBounds());
            HN.Callbacks.mergeCells(layout.currentSheet().path(),bounds);
        });

        $("#inlinedropdown").bind("mousedown", function (e) {
