...
*/
    ...
    // TODO fix how repeating arguments are handled: only arguments
    // tagged as type == "infinite", but this means that we have to
    // re-number arguments that follow!
	...
	var b  = api.getArgumentCount(), // base
	    ...
	    if (f.repeat && (i == (f.args.length-1))) {
		$(".wizrep").remove(); // remove existing more buttons
		var more = $( document.createElement('span') ).attr("id", "wizrep" + i).attr("class", "wizrep").attr("title", "more").html("+")
		    .click(function(e) { api.wizardAddArguments(f); });
		e.append(more);
	    }

            as.append(e);

	    // if added arguments are outwith the scrolled view,
	    // scroll down.

	    if (b>0) {
		if ((e.position().top + e.height() + as.scrollTop()) > (as.height() + as.scrollTop())) {
		  as.scrollTop(as.scrollTop() + e.height() + 4);
		}
	    }
