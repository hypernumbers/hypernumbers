...
*/
    ...
    api.getFunctionText = function(f) {
	  ...
	  if (f.args[i].type == "variable") {
	      ...
	      args.push(api.getArgValues(f.args[i]));
	  }
	...
	var l = $( document.createElement('label') ).text(a.name).attr("title", a.desc).attr("for", n);
	...
	switch (a.type) {
 	    ...
 	    case "variable":
	      ...
	      var x = api.createArgument(a.attr,f);

	      // TODO This should be .after(), not .append(), but we
	      // cannot use .after() as the markup has not yet been
 	    ...
 	    case "finite":
	      ...
	      var req = $( document.createElement('span') ).attr("id", "r" + n).attr("class", "req").attr("title", "required");
	      e.append(req);
 	      break;
	    ...
	    if (a.variable) { a.variable.val(nc[1]); }
    ...
    api.createResizer = function(name,f) {
	...
	var z = $( document.createElement('input') ).attr("id", n).attr("size", 5).attr("value", 1).attr("type", "number");
	...
	if (f.resize) {
		...
		d.append(api.createResizer("Rows",f));
	    }
	    if ((f.resize == "column") || (f.resize == "range")) {
		d.append(api.createResizer("Columns",f));
	    }
	}

        for(var i=0;i<f.args.length;i++) {
	    d.append(api.createArgument(f.args[i], f));
	}
	api.setWizText(api.getFunctionText(f));
    };
