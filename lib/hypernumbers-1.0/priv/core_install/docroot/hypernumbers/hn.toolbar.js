...
*/
	...
	api.adjust_arglist = function(dialog, content, bottom, footer) {
		...
		var g = function(x) {
		    if ((x != null) && 
			(typeof(x.expression.fn.name) != "undefined")) {
			var n = x.expression.fn.name;
			$("#wizfunlist option").removeAttr('selected').
			    each(function(i) {
				     if ($(this).text() == n.toUpperCase()) {
					 $(this).attr('selected', 'selected');
					 $("#wizfunlist").trigger("click");

			// TODO populate with args for function
		...
		api.parseExpression(layout.tabUI().currentCellFormula(), g);
	    }
        });
