...
HN.ToolBar = function (layout) {
    ...
    api.loadViews = function(permissions, groups) {
        ...
        clickPublic = function() {
            ...
            HN.Callbacks.setView(path, view, perms.views[view].everyone, 
        ...
        $(".groupinput").bind("change", function() {
            ...
            if (checked) {
                ...
                pv.groups = HN.Util.remove(pv.groups, a[1]);
            };
            hn.data.writeKey(path, "permissions", perms);
            HN.Callbacks.setView(path, a[0], perms.views[a[0]].everyone, 
                                 perms.views[a[0]].groups, null);
        });
        ...
        if (hn.data.is_admin) {
            ...
            advusers = HN.Util.makeAdvUsers(permissions),
    ...
    function enableSiteMenu() {
        ...
        $("#sitemenulist a").bind("mousedown", function(e) {
            console.log($(e.currentTarget).attr("data-tab"));
            e.preventDefault();
        ...
        $("#addgroup").bind("click", function(data) {
            ...
            hide = function (e) {
                if (HN.Util.is_inside(e.target, $("#addgroupmenu")[0])) {
                    var Groups = [],
                    Everyone   = false;
                    Groups = perms.views[perms.champion].groups,
                    Groups.push($(e.target).text());
                    Everyone = perms.views[perms.champion].everyone;
                    HN.Callbacks.setView(path, perms.champion, Everyone, Groups, null);
                };
