...
HN.GridRender = function () {
    ...
    notify = function () {
        alert("The vixo spreadsheet does not support your browser yet." +
    ...
    api.bindControls = function () {
        ...
        mfn = function (e) {
            ...
            url = document.location.pathname + $(button.parent()).attr("data-ref");
            $.ajax({
                "type"     : "POST",
                "url"      : url,
                "dataType" : "json",
    ...
    api.render = function (sheet, pageref) {
        ...
        tmpheight, totalHeight = 0, totalWidth = 0, ghostWidth = 0, tmpWidth,
        index, inp, ghost, ref, div, inlcss, outerpadding;
        ...
        for (y in data.data.cell) {
            if (data.data.cell.hasOwnProperty(y)) {
                ...
                for (x = 0; x < index.length; x++) {
                    ...
                    if (data.data.cell[y].hasOwnProperty(x)) {
                        if (pageref.type === "path" ||
                            ...
                            if ((ghost === true && view === "webpage") ||
                                (ghost === true && inp === "none" && 
                                 view === "wikipage")) {
                                ghostWidth = ghostWidth + width; 
                            } else {
                                ghostWidth = 0;
                            }
                ...
                if ((tmpWidth - ghostWidth) > totalWidth) {
                    totalWidth = (tmpWidth - ghostWidth);
                }
            }
