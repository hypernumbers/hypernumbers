...
/*global alert: false,  $: false, Y: false, X: false, localStorage: false, escape: false, unescape: false, window: false, jscolor: false */
var hn = {},
    HN = {},
    lang;
...
HN.GridRender = function () {
    ...
    api.render = function (sheet) { 
        ...
        for (y in data.data.cell) {
            if (data.data.cell.hasOwnName(y)) {
                tmpheight = 0;
                tmpWidth  = 0;
                
                for (x in data.data.cell[y]) {
                    if (data.data.cell[y].hasOwnName(x)) {
