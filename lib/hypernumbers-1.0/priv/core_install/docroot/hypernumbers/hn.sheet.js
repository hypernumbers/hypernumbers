...
HN.Sheet = function (path, data) {
    ...
    api.cellChanged = function(y, x) {
        ...
        var row         = api.pageData().data.row[y],
            cell        = api.cell(y, x);
            fixedHeight = row && row[y].fixedHeight;
        
        if (!fixedHeight && (!cell.merge || cell.merge.down === 0)) {
            
            var oldHeight = api.row_height(y),
                height    = api.calc_height(y);
            
            if (Math.abs(height - oldHeight) > 2) {
                HN.Callbacks.setHeight(path, y, height);
            }
        }
    };
    ...
    api.calc_height = function (row) {
        ...
        var hidden = HN.Util.id("hidden_input"),
            cell, width, val, style, i,
            height = HN.Sheet.CELL_HEIGHT;
        ...
        for (i in data.data.cell[row]) {
            ...
            val    = cell.value || "";
            style  = api.lookupCSS(row, i);

            hidden.setAttribute("style", style + "width:" + width +
                                "px;overflow:visible;");
    ...
    api.processLoaded = function () {
        ...
        for (y in data.data.cell) {
            ...
            if (y) {
                ...
                var row         = api.pageData().data.row[y],
                    newHeight   = null,
                    oldHeight   = api.row_height(y),
                    fixedHeight = row && row[y].fixedHeight;

                if (!fixedHeight) { 
                    newHeight = api.calc_height(y);
                    if (Math.abs(newHeight - oldHeight) > 2 &&
                        typeof HN.Callbacks !== "undefined") {
                        HN.Callbacks.setHeight(path, y, newHeight);
                    }
                } else {
                    newHeight = oldHeight;
                }
