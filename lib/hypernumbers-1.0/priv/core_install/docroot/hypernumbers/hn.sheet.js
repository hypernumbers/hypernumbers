...
HN.Sheet = function (path, data) {
    ...
    api.calc_height = function (rowIndex) {
        ...
        for (i in data.data.cell[rowIndex]) {
            ...
            if (cell && !cell.heightCalced) {

                cell.calcedHeight = true;
                ...
                if (hidden.offsetHeight + 2 > nHeight &&
                    (!cell.merge || cell.merge.down === 0)) {
                    
                    nHeight = hidden.clientHeight + 2;
                }
            }
        }
        
        if (Math.abs(oldHeight - nHeight) > 2 &&
