...
HN.Callbacks.pasteValues = function(path, range, srcurl, srcrange) {
    var src = srcurl + HN.Util.range_to_str(srcrange);
    console.log("in pasteValues");
    HN.Util.postRange(path, range, {"copyvalue": {"src": src}});
};

HN.Callbacks.pasteRange = function(path, range, srcurl, srcrange) {
    console.log("in pasteRange");
    var src = srcurl + HN.Util.range_to_str(srcrange);
