%%%-------------------------------------------------------------------
%%% File        : spriki_funs.erl
%%% Author      : Benefits Dragon <bd@vixo.com>
%%% Description : Spreadsheet function to match those provided in
%%%               MS Excel.
%%%
%%% Created     : 10 Dec 2006 by Benefits Dragon <bd@vixo.com>
%%%-------------------------------------------------------------------
-module(stdfuns).

-include("spriki.hrl").

-define(EXPANSION_DETAIL, 20). % For Taylor approximations.

% Used in: floor, ceiling, even.
% Can't use arbitrary functions in guards, hence using a macro.
-define(is_multiple(X, Multiple), (erlang:trunc(X / Multiple) * Multiple) == (X * 1.0)).
...
%% Microsoft API - these functions are all identical to those in Excel 97
-export([get_cell/3,http/1,https/1]).
...
%% these functions are all our extensions to the Microsoft Excel 97 API
%%-export([quoted_to_string/1]).

%%%-----------------------------------------------------------------------------
%%%
%%% This function handles getting the value of a cell from a remote site
%%%
%%%-----------------------------------------------------------------------------

%% This function does not seem to be used *anywhere*.
get_cell(Site,Path,Cell)->
    io:format("in spriki_funs:get_cell got to here~n"),
    Site2="http://"++Site,
    {X,Y}=util2:strip_ref(Cell),
    spriki:remote_calc(Site2,Path,X,Y,new).

http(Path)->
    io:format("in spriki:http with Path of ~p~n",[Path]),
    Path.

https(Path)->
    io:format("in spriki:https with Path of ~p~n",[Path]),
    Path.
...
%%        ends of limits.
%%
%%        Function indirect/2 is only indirect/1 as we don't have
%%        different cell reference types.
%%
...
%%
%% *******************************************************************
%% Currently 228 functions implemented
%% (some duplicates ie different arities!)
%% *******************************************************************
%%====================================================================
